...
function GraphPageContent() {
  ...
  if (error) return <ErrorState title="数据加载失败" description={error.message} />;
    ...
    <div className="flex h-full flex-col gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {focusNodeId && (
            <button
              ...
              className="ml-2 inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-medium text-indigo-700 hover:bg-indigo-100 transition-colors dark:bg-indigo-900/30 dark:text-indigo-300 dark:hover:bg-indigo-900/50"
            >
              局部视图 · {depthLimit} 跳
              <X className="h-3 w-3" />
            </button>
      ...
      <div className="flex flex-1 flex-col md:flex-row gap-4 min-h-0">
        ...
        <div className="flex-1 rounded-lg border border-slate-200 bg-white shadow-sm overflow-hidden relative order-1 md:order-none dark:border-slate-700 dark:bg-slate-900 min-h-[300px]">
          ...
          <div className="absolute top-2 right-2 z-20 flex items-center gap-1 rounded-lg border border-slate-200 bg-white/90 p-0.5 shadow-sm backdrop-blur-sm dark:border-slate-600 dark:bg-slate-800/90">
            {LAYOUT_OPTIONS.map((opt) => {
              const Icon = opt.icon;
              const active = graphLayout === opt.value;
              return (
                <button
                  key={opt.value}
                  onClick={() => setGraphLayout(opt.value)}
                  className={`rounded-md p-1.5 text-xs transition-colors ${active ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300' : 'text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300'}`}
