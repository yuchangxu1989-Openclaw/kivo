...
const EDGE_TYPES = [
  ...
  { value: 'prerequisite_of', label: '前置知识', color: '#0891B2' },
  { value: 'confusable_with', label: '易混淆', color: '#DC2626' },
  { value: 'explains', label: '解释说明', color: '#059669' },
  { value: 'assesses', label: '评估检验', color: '#D97706' },
  { value: 'solved_by', label: '解题方法', color: '#7C3AED' },
  { value: 'derived_from', label: '推导来源', color: '#0284C7' },
  { value: 'proves', label: '证明依据', color: '#65A30D' },
  { value: 'applies_to', label: '适用范围', color: '#0D9488' },
  { value: 'belongs_to', label: '所属主题', color: '#4F46E5' },
];
...
function GraphPageContent() {
  ...
  // API URL — use snapshot endpoint when timeline is active
  const apiUrl = useMemo(() => {
    if (timelineDate) {
      return `/api/v1/graph/snapshot?before=${timelineDate}`;
    }
  ...
  const applyGraphView = useCallback((view: 'topic' | 'conflict' | 'reference') => {
    setShowControls(false);
    setFocusNodeId(null);
    setSelectedNode(null);
    if (view === 'topic') {
      setVisibleNodeTypes(new Set());
      setVisibleEdgeTypes(new Set(['supports', 'depends_on', 'semantic_neighbor', 'co_occurs']));
      setGraphLayout('radial');
      return;
    }
    if (view === 'conflict') {
      setVisibleNodeTypes(new Set());
      setVisibleEdgeTypes(new Set(['conflicts']));
      setGraphLayout('force');
      return;
    }
    setVisibleNodeTypes(new Set());
    setVisibleEdgeTypes(new Set(['supports', 'depends_on', 'co_occurs']));
    setGraphLayout('hierarchy');
  }, []);
  ...
  if (error) return (
    <div className="flex h-full flex-col items-center justify-center gap-4 p-8">
      ...
      <div className="text-center space-y-2">
        <h2 className="text-lg font-semibold text-slate-900">图谱数据加载失败</h2>
        <p className="text-sm text-slate-500 max-w-md">{error.message || '无法获取图谱数据，请检查网络连接。'}</p>
      </div>
      <button
        onClick={handleRetry}
        disabled={retrying}
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold text-slate-900">知识图谱</h1>
          {snapshot && (
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={() => applyGraphView('topic')} className="inline-flex items-center gap-1.5 rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-xs font-medium text-cyan-700 transition-colors hover:bg-cyan-100"><Orbit className="h-3.5 w-3.5" />主题视角</button>
          <button type="button" onClick={() => applyGraphView('conflict')} className="inline-flex items-center gap-1.5 rounded-full border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs font-medium text-rose-700 transition-colors hover:bg-rose-100"><AlertTriangle className="h-3.5 w-3.5" />冲突视角</button>
          <button type="button" onClick={() => applyGraphView('reference')} className="inline-flex items-center gap-1.5 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-medium text-indigo-700 transition-colors hover:bg-indigo-100"><Radio className="h-3.5 w-3.5" />引用视角</button>
          <button
            onClick={() => setShowControls((v) => !v)}
            className={`rounded-full border border-slate-200 p-2 transition-colors ${showControls ? 'bg-cyan-100 text-cyan-700' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-600'}`}
            aria-label="切换控制面板"
          >
      ...
      {/* Timeline Slider (FR-FIX-08) */}
      {snapshot && snapshot.nodes.length > 0 && (() => {
        const dates = snapshot.nodes.map(n => n.createdAt).filter(Boolean).sort();
        const minDate = dates[0];
        if (!minDate) return null;
        return (
          <TimelineSlider
            minDate={minDate}
            value={timelineDate || undefined}
      ...
      <div className="flex flex-1 flex-col md:flex-row gap-4 min-h-0">
        ...
        {showControls && !isFocus && (
          <div className="order-2 md:order-none md:w-64 md:shrink-0 md:overflow-y-auto max-h-64 md:max-h-none overflow-y-auto grid grid-cols-1 gap-4 md:space-y-4 md:gap-0">
            {/* Search */}
            <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
              ...
              {graphSearch && searchHighlightIds.size === 0 && (
                <p className="mt-1.5 text-[10px] text-slate-500">无匹配节点</p>
              )}
            </div>
        ...
        {/* Center: Graph */}
        <div className="relative order-1 min-h-[560px] flex-1 overflow-hidden rounded-xl border border-slate-200 bg-slate-50 shadow-sm md:order-none">
          {/* Layout toggle removed — force layout only */}
          {/* Shift+drag hint */}
          <div className="absolute bottom-2 left-2 z-20">
            <span className="inline-flex items-center gap-1 rounded-md bg-slate-100/80 px-2 py-0.5 text-[10px] text-slate-400 backdrop-blur-sm">
        ...
        {selectedNode && (
          <Card className="order-3 max-h-80 w-full overflow-y-auto border-slate-200 bg-white shadow-sm md:order-none md:max-h-none md:w-80 md:shrink-0">
            <CardHeader className="pb-2 flex flex-row items-start justify-between">
              <CardTitle className="text-sm font-medium text-slate-900">{selectedNode.title}</CardTitle>
              <button
                onClick={() => {
                  setSelectedNode(null);
                  setFocusNodeId(null);
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selectedNode.domain && (
                  <span className="inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] text-slate-600">
                    {selectedNode.domain}
                  </span>
                )}
              ...
              <div className="space-y-1 rounded-xl border border-slate-200 bg-slate-50 p-3 text-[11px] text-slate-500">
                <p>来源：{selectedNode.sourceRef || '未标注'}</p>
                <p>被引用：{selectedNode.connectionCount ?? selectedNode.relations?.length ?? 0} 次</p>
                <p>引用任务：Agent 会话、规则注入、知识召回</p>
              </div>
                  ...
                  <div className="max-h-40 overflow-y-auto space-y-1">
                    {selectedNode.relations.map((rel) => (
                      <button
                        ...
                        className="flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-left text-xs text-slate-600 hover:bg-slate-50 transition-colors"
                      >
                        {rel.direction === 'outgoing' ? (
                          <ArrowRight className="h-3 w-3 shrink-0 text-slate-400" />
                        ) : (
                          <ArrowLeft className="h-3 w-3 shrink-0 text-slate-400" />
                        )}
                        <span
              ...
              <button
                ...
                className="w-full rounded-md bg-indigo-50 px-3 py-1.5 text-xs font-medium text-indigo-700 hover:bg-indigo-100 transition-colors inline-flex items-center justify-center gap-1.5"
              >
      ...
      <CognitivePanel visible={isOverview}>
        {snapshot && (
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <p className="mb-3 text-xs font-medium uppercase tracking-wider text-slate-500">节点类型分布</p>
            <MiniBarChart
              width={240}
              height={56}
      ...
      {pendingLink && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-sm rounded-lg border border-slate-200 bg-white p-5 shadow-xl">
            <div className="flex items-center gap-2 mb-3">
              <Link2 className="h-4 w-4 text-indigo-500" />
              <h3 className="text-sm font-medium text-slate-900">创建链接？</h3>
            </div>
            <p className="text-xs text-slate-500 leading-relaxed mb-4">
              将在源节点内容中追加 <code className="rounded bg-slate-100 px-1 py-0.5 text-[10px]">[[目标标题]]</code> 链接。
            </p>
