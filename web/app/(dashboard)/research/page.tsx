          ...
          <div className="space-y-2">
            ...
            {task.filledGapTopic ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-600">{task.filledGapTopic}</span>
                <Link
                  href="/gaps"
                  className="inline-flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-800"
                >
...
export default function ResearchPage() {
  ...
  async function adoptTask(id: string) {
    setSubmitting(true);
    try {
      await apiFetch(`/api/v1/research/${id}/adopt`, { method: 'POST' });
      setNotice('已采纳，报告内容正在拆解入库。');
      await mutate();
    } finally {
      setSubmitting(false);
    }
  ...
  async function toggleHighlight(id: string, highlighted: boolean) {
    setSubmitting(true);
    try {
      await apiFetch(`/api/v1/research/${id}`, {
        method: 'PATCH',
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {!task.adopted && (
                        <Button
                          ...
                          aria-label={`采纳调研报告 ${task.topic}`}
                        >
                          <CheckCircle2 className="mr-1.5 h-4 w-4" />采纳入库
                        </Button>
                      )}
                      {task.adopted && (
                        <Badge variant="secondary" className="bg-emerald-100 text-emerald-700">已采纳</Badge>
                      )}
                      <Button
                        variant={task.highlighted ? 'secondary' : 'outline'}
                        size="sm"
                        disabled={submitting}
                        onClick={() => toggleHighlight(task.id, !task.highlighted)}
                        aria-label={task.highlighted ? '取消重点标记' : '标记为重点'}
                      >
                        <Star className={`mr-1.5 h-4 w-4 ${task.highlighted ? 'fill-amber-400 text-amber-500' : ''}`} />
                        {task.highlighted ? '重点' : '标记重点'}
                      </Button>
                    </div>
                  )}
