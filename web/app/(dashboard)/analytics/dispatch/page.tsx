...
export default function DispatchAnalyticsPage() {
  ...
  const unhandledAlertCount = analytics?.unhandledAlertCount ?? alerts.length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="border-slate-200/80 bg-white/95 shadow-sm">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg">未处理告警</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-semibold text-rose-600">{unhandledAlertCount}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="space-y-4">
                  ...
                  {alerts.slice(0, 3).map((alert) => (
                    <div key={alert.id} className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <p className="text-sm font-medium text-rose-900">{alert.ruleId}</p>
                        <Badge variant="destructive">未处理</Badge>
                      </div>
                      <p className="mt-1 text-sm leading-6 text-rose-700">{alert.error}</p>
                    </div>
                  ))}
                </div>
              )}
              {failedRules.length === 0 && alerts.length === 0 ? (
                <EmptyState
                  icon={TriangleAlert}
                  title="最近没有分发失败"
          ...
          <Card className="border-slate-200/80 bg-slate-50 shadow-sm">
            <CardContent className="flex items-center gap-3 p-6 text-sm leading-6 text-slate-600">
              <Send className="h-5 w-5 shrink-0 text-indigo-600" />
