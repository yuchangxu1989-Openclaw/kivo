...
function parseTextFile(file: File, text: string): ImportCandidate[] {
  return parsePlainTextFile(file, text);
}
  ...
  return items
    ...
    .map((item, i) => ({
      ...
      sourceRange: {
        ...
        paragraph: i + 1,
        section: `JSON 条目 ${i + 1}`,
        originalText: typeof item.content === 'string' ? item.content : '',
      },
      sourceParagraphs: typeof item.content === 'string' ? [{ index: i + 1, text: item.content, highlighted: true }] : undefined,
  ...
  return lines.slice(1)
    .map((line, i) => {
      ...
      return {
        ...
        sourceParagraphs: [{ index: i + 2, text: line, highlighted: true }],
        status: 'pending' as const,
      };
    })
    .filter(c => c.content.trim().length > 0);
...
function SourceContextPanel({ candidate, onClose }: { candidate: ImportCandidate; onClose: () => void }) {
  const paragraphs = candidate.sourceParagraphs ?? [{ index: candidate.sourceRange?.start ?? 1, text: candidate.sourceContext || candidate.content, highlighted: true }];
  const virtualized = paragraphs.length > 100;
  const visibleParagraphs = virtualized ? paragraphs.filter((item) => item.highlighted).slice(0, 8) : paragraphs;

  return (
    <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50/50 p-3 text-xs text-slate-700">
      <div className="mb-2 flex items-center justify-between gap-2">
        <span className="font-medium text-amber-700">原文上下文 · {candidate.sourceLocation || candidate.sourceAnchor}</span>
        <button type="button" onClick={onClose} className="text-slate-500 hover:text-slate-800">关闭</button>
      </div>
      {virtualized && (
        <p className="mb-2 rounded bg-white/70 px-2 py-1 text-[11px] text-amber-700">
          文档超过 100 段，只加载命中段落附近内容。
        </p>
      )}
      <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
        {visibleParagraphs.map((paragraph) => (
          <div key={`${candidate.id}-${paragraph.index}`} className={`rounded-md border px-3 py-2 ${paragraph.highlighted ? 'border-amber-300 bg-amber-100 text-slate-900 shadow-sm' : 'border-slate-200 bg-white/80'}`}>
            <span className="mb-1 block text-[10px] font-medium text-slate-500">第 {paragraph.index} 段</span>
            <p className="whitespace-pre-wrap leading-5">{paragraph.text}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 space-y-1">
          ...
          <button type="button" className="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-800 hover:underline cursor-pointer bg-transparent border-none p-0" onClick={() => setSourceExpanded(!sourceExpanded)} aria-expanded={sourceExpanded}>
            <FileText className="h-3 w-3" /> {candidate.sourceLocation || candidate.sourceAnchor}
          </button>
          {sourceExpanded && <SourceContextPanel candidate={candidate} onClose={() => setSourceExpanded(false)} />}
        </div>
      </div>
      {candidate.status === 'pending' && (
...
export default function DocumentImportPage() {
  ...
  // Load existing import jobs from API on mount (persisted within server session)
  useEffect(() => {
    apiFetch<{ data: ImportJob[] }>('/api/v1/imports')
      .then(res => {
        if (res.data && res.data.length > 0) {
          setJobs(prev => {
            const existingIds = new Set(prev.map(j => j.id));
            const newOnes = res.data.filter(j => !existingIds.has(j.id));
            return newOnes.length > 0 ? [...prev, ...newOnes] : prev;
          });
        }
      })
      .catch(() => { /* ignore — page still works with local state */ });
  }, []);
      ...
      try {
        await apiFetch('/api/v1/knowledge', {
          ...
          body: JSON.stringify({
            ...
            status: 'active',
            confidence: 0.85,
            sourceDocument: c.sourceDocument || job.fileName,
            sourceLocation: c.sourceLocation || c.sourceAnchor,
            sourceOriginalText: c.sourceRange?.originalText || c.sourceContext || c.content,
            sourceRange: {
              documentId: c.sourceRange?.documentId || c.sourceDocument || job.fileName,
              page: c.sourceRange?.page,
              paragraph: c.sourceRange?.paragraph ?? (c.sourceRange?.kind === 'paragraph' || c.sourceRange?.kind === 'line' ? c.sourceRange.start : undefined),
              section: c.sourceRange?.section || c.sourceLocation || c.sourceAnchor,
              originalText: c.sourceRange?.originalText || c.sourceContext || c.content,
            },
          }),
        });
        success++;
    ...
    <div className="space-y-6">
      ...
      {jobs.length === 0 ? (
        <div className="space-y-4">
          <EmptyState icon={FileText} title="暂无导入记录" description="上传文档后，系统自动提取知识候选条目。你可以逐条审核后导入知识库。" />
          <p className="text-center text-sm text-slate-500">
            已有多媒体材料？前往{' '}
            <Link href="/wiki/materials" className="font-medium text-indigo-600 hover:text-indigo-500 underline">材料库</Link>
            {' '}查看上传的 PDF、图片、视频和音频。
          </p>
        </div>
      ) : (
