...
const NATURE_KEYS = ['fact', 'intent', 'decision', 'methodology'] as const;
const NATURE_LABELS: Record<string, string> = {
  fact: '事实', intent: '意图', decision: '决策', methodology: '方法论',
};
...
function TableView({ entries, sort, onSort, onPatch, selectedIds, onToggle }: { entries: KnowledgeEntry[]; sort: string; onSort: (s: string) => void; onPatch: (id: string, field: string, value: string) => void; selectedIds: Set<string>; onToggle: (id: string) => void }) {
  const cols = [
    ...
    { label: '置信度', sortKey: '-confidence', responsive: 'hidden md:table-cell' },
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-800/80">
            {cols.map(c => (
              <th key={c.label} className={`px-4 py-3 text-left font-medium text-slate-600 dark:text-slate-300 ${c.responsive}`}>
                {c.sortKey ? (
          ...
          {entries.map(e => (
            <tr key={e.id} className={`border-b border-slate-50 transition-colors hover:bg-slate-50/50 dark:border-slate-800 dark:hover:bg-slate-800/50 ${selectedIds.has(e.id) ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : ''}`}>
              ...
              <td className="max-w-[120px] px-4 py-3">
                <Link href={`/knowledge/${e.id}`} title={e.content.trim()} className="font-medium text-slate-800 hover:text-indigo-600 hover:underline dark:text-slate-200 dark:hover:text-indigo-400 truncate block">{e.title || extractTitle(e.content)}</Link>
              </td>
              <td className="max-w-[240px] px-4 py-3">
                <span className="text-xs text-slate-600 dark:text-slate-400 line-clamp-1">{e.content.length > 50 ? e.content.slice(0, 50) + '...' : e.content}</span>
              </td>
              ...
              <td className="px-4 py-3 text-muted-foreground hidden md:table-cell">{e.confidence != null ? `${(e.confidence * 100).toFixed(0)}%` : '—'}</td>
...
function KnowledgeListPageInner() {
  ...
  const [statusTab] = useState('active');
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-2">
            ...
            <p className="max-w-3xl text-sm leading-6 text-muted-foreground">
              {meta?.total != null ? `共 ${meta.total} 条活跃知识` : '加载中...'}
            </p>
          </div>
        ...
        <CardContent className="space-y-3">
          <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <Select value={nature || 'all-natures'} onValueChange={v => { setNature(v === 'all-natures' ? '' : v); setPage(1); }}>
              <SelectTrigger className="w-full" aria-label="按知识性质筛选"><SelectValue placeholder="全部性质" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all-natures">全部性质</SelectItem>
                {NATURE_KEYS.map(k => <SelectItem key={k} value={k}>{NATURE_LABELS[k]}</SelectItem>)}
              </SelectContent>
            </Select>
