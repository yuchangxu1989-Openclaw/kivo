...
function inferEventType(entry: KnowledgeEntry): { label: string; icon: typeof FilePlus2; accent: string } {
  ...
  if (diff < 1000) {
    return { label: '创建', icon: FilePlus2, accent: 'bg-emerald-500' };
  }
  return { label: '修改', icon: FileEdit, accent: 'bg-sky-500' };
}
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-3">
          <h1 className="text-3xl font-semibold tracking-tight text-slate-950">知识时间线</h1>
      ...
      <CognitivePanel visible={isOverview}>
        <div className="rounded-2xl border border-slate-200/80 bg-white/95 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-sm font-medium text-slate-700">每日活动量</p>
      ...
      {playbackEnabled && entries.length > 0 && (
        <div className="space-y-4">
          ...
          {filteredGraph && filteredGraph.nodes.length > 0 && (
            <div className="h-64 overflow-hidden rounded-xl border border-slate-200/80">
                ...
                {items.map((entry) => {
                  ...
                  return (
                    <div key={entry.id} className="group relative">
                      <div className="absolute -left-[calc(2rem+5px)] top-3 h-2.5 w-2.5 rounded-full border-2 border-white bg-slate-400" />
                      <Link
                        href={`/knowledge/${entry.id}`}
                        className="block rounded-xl border border-slate-200/80 bg-white/95 p-4 shadow-sm transition-all hover:border-slate-300 hover:shadow-md"
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              <span className="line-clamp-1 text-sm font-medium text-slate-950">
                                {entry.title || '未命名'}
                              </span>
