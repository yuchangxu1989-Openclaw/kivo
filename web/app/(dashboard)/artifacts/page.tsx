    ...
    <div className="space-y-4">
      {pending.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-slate-700">待审核 ({pending.length})</h4>
          {pending.map(c => (
            <div key={c.id} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-3">
              <div className="flex-1 min-w-0">
                ...
                {editingId === c.id ? (
                  ...
                  <p className="text-sm text-slate-800 truncate">{c.label}</p>
                )}
      ...
      {decided.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-slate-500">已审核 ({decided.length})</h4>
          {decided.map(c => {
            const decision = artifact.candidateDecisions.find(d => d.candidateId === c.id);
            return (
              <div key={c.id} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/50 p-3">
                <div className="flex-1 min-w-0">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground">{c.section}</span>
                  <p className="text-sm text-slate-600 truncate">{decision?.editedValue || c.label}</p>
                </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold tracking-tight text-slate-950">分析产物审核</h1>
      ...
      {artifacts.length === 0 ? (
        ...
        <div className="space-y-3">
          {artifacts.map(art => (
            <Card
              key={art.id}
              className={`cursor-pointer transition-all hover:border-indigo-200 ${expandedId === art.id ? 'border-indigo-300 ring-1 ring-indigo-200' : ''}`}
