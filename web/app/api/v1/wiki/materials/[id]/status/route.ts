...
function normalizePipelineStatus(input: MaterialPipelineStatus, fallback: string): UiPipelineStatus {
  ...
  if (input === 'in_progress' || input === 'processing') return 'extracting';
  ...
  try {
    ...
    return NextResponse.json({
      data: {
        ...
        processedChunks: material.processedChunks,
        progress: material.totalChunks
          ? Math.round((material.processedChunks / material.totalChunks) * 100) / 100
          : null,
