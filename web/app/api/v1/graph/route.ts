...
export async function GET(request: NextRequest) {
  try {
    ...
    const allNodeIds = new Set(dtoNodes.map((n) => n.id));
    const rawEdges = getGraphEdges(allNodeIds);

    const allEdges: GraphEdgeDTO[] = rawEdges.map((e) => ({
      id: String(e.id),
    ...
    // If too many nodes, select top-N by connection count to keep graph readable
    let selectedNodes: GraphNodeDTO[];
    let selectedEdges: GraphEdgeDTO[];

    if (dtoNodes.length > limit) {
      ...
      const sorted = [...dtoNodes].sort((a, b) => (degree.get(b.id) ?? 0) - (degree.get(a.id) ?? 0));
      selectedNodes = sorted.slice(0, limit);
      const selectedIds = new Set(selectedNodes.map((n) => n.id));
      selectedEdges = allEdges.filter((e) => selectedIds.has(e.sourceId) && selectedIds.has(e.targetId));
    } else {
      selectedNodes = dtoNodes;
      selectedEdges = allEdges;
    }

    // Compute lightweight insights from selected data (O(N+E), not O(N²))
    const degree = new Map<string, number>();
    for (const n of selectedNodes) degree.set(n.id, 0);
    for (const e of selectedEdges) {
      degree.set(e.sourceId, (degree.get(e.sourceId) ?? 0) + 1);
      degree.set(e.targetId, (degree.get(e.targetId) ?? 0) + 1);
    }
    const isolatedNodeIds = selectedNodes.filter((n) => (degree.get(n.id) ?? 0) === 0).map((n) => n.id);

    // Bridge detection via Tarjan's algorithm — O(N+E)
    const bridgeNodeIds = findBridgeNodes(selectedNodes, selectedEdges);
    ...
    const response: ApiResponse<GraphSnapshotDTO> = {
      data: {
        ...
        meta: { totalNodes: dtoNodes.length, totalEdges: allEdges.length, displayedNodes: selectedNodes.length },
      },
    };
