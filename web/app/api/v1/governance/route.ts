...
import { ensureOperationalTables, openOperationalDb } from '@self-evolving-harness/kivo';
...
function loadGovernanceSnapshotReports(): GovernanceReport[] {
  const db = openOperationalDb({ dbPath: process.env.KIVO_DB_PATH, cwd: process.cwd() });
  try {
    ensureOperationalTables(db);
    const rows = db.prepare(`
      SELECT operation_id, operation_type, before_state_json, created_at
    ...
    return rows.map((row) => {
      const beforeState = JSON.parse(row.before_state_json) as unknown[];
      const title = row.operation_type === 'merge'
        ? `意图合并操作 ${row.operation_id}`
        : `意图权重调整 ${row.operation_id}`;
      return {
        id: row.operation_id,
        title,
        summary: `可按快照回退 ${beforeState.length} 条意图。`,
        status: 'completed' as const,
        issuesFound: beforeState.length,
        issuesResolved: beforeState.length,
...
export async function GET() {
  try {
    ...
    const snapshotReports = loadGovernanceSnapshotReports();
    const mergedData: GovernanceData = {
      ...data,
      reports: snapshotReports.length > 0 ? snapshotReports : data.reports,
    };
