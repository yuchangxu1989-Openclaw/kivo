...
const RELATION_TYPE_LABELS: Record<string, { label: string; color: string }> = {
  related: { label: '相关', color: 'bg-blue-100 text-blue-800' },
  references: { label: '引用', color: 'bg-amber-100 text-amber-800' },
  contradicts: { label: '矛盾', color: 'bg-red-100 text-red-800' },
  supports: { label: '支持', color: 'bg-green-100 text-green-800' },
  depends_on: { label: '依赖', color: 'bg-purple-100 text-purple-800' },
  co_occurs: { label: '共现', color: 'bg-teal-100 text-teal-800' },
};
        ...
        <div className="space-y-3">
          ...
          <ul className="max-h-60 space-y-1 overflow-y-auto">
            {candidates.slice(0, 20).map((entry) => (
              <li key={entry.id}>
                <button
                  className="w-full rounded-lg px-3 py-2 text-left text-sm transition-colors hover:bg-slate-100 disabled:opacity-50"
                  disabled={saving}
      ...
      <CardContent>
        {relations.length === 0 ? (
          ...
          <ul className="space-y-2">
            {relations.map((rel, i) => (
              <li key={`${rel.targetId}-${i}`} className="flex items-center gap-2 rounded-lg px-2 py-1.5 transition-colors hover:bg-slate-50">
