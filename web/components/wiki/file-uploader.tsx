...
export interface UploadedMaterialPayload {
  fileId: string;
  fileName: string;
  fileSize: number;
  status: 'processing';
...
export function FileUploader({
  ...
  onUploaded?: (uploaded?: UploadedMaterialPayload & { mimeType: string; spaceId: string }) => void;
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      const payload = await new Promise<UploadedMaterialPayload>((resolve, reject) => {
        ...
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            try {
              const body = JSON.parse(xhr.responseText) as UploadedMaterialPayload;
              resolve(body);
            } catch {
          ...
          try {
            ...
            reject(new Error(errBody.error?.message || `上传失败: ${xhr.status}`));
          } catch {
