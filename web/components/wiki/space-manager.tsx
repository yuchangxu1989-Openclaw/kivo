...
export function SpaceManager() {
  ...
  useEffect(() => {
    if (!selectedSpaceId && visibleSpaces.length > 0) {
      ...
      setSelectedDirectoryId(visibleSpaces[0].id);
    }
    ...
    <div className="space-y-6 bg-white text-black">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-600">目录承载章节分类，条目承载具体知识点。搜索调用后端语义检索，不做前端关键词假冒搜索。</p>
        </div>
              ...
              <Card className="border-slate-200 bg-white shadow-sm">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => openDirectoryDialog({ mode: 'create', parentId: selectedSpace.id, parentTitle: selectedSpace.title })}>
                    ...
                    <div className="space-y-2">
                      {entries.map((entry) => (
                        <button
                          ...
                          draggable
                          onDragStart={(event) => {
                            event.dataTransfer.setData('application/json', JSON.stringify({ ...entry, type: 'page', children: [] }));
                            event.dataTransfer.effectAllowed = 'move';
                          }}
                          onClick={() => setSelectedPageId(entry.id)}
                          className={`w-full rounded-2xl border px-3 py-3 text-left ${entry.id === selectedPageId ? 'border-sky-300 bg-sky-50' : 'border-slate-200 bg-white hover:border-slate-300'}`}
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-slate-400">
                                ...
                                {entry.matchReason && <span>{entry.matchReason}</span>}
                              </div>
                            </div>
                          </div>
                        </button>
                ...
                {!selectedPageId ? (
                  ...
                  <div className="space-y-5 p-6">
                    ...
                    <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                      <span>图谱节点可用于回溯关联关系。</span>
                      <Link href={pageData.data.metadata?.extra?.graphNodeHref || `/graph?focus=${pageData.data.id}`} className="font-medium text-sky-700 hover:text-sky-900">
                        查看关联图谱
                      </Link>
                    </div>
                  </div>
