...
const secondaryNavItems = [
  ...
  { href: '/settings/intents', label: '意图库', icon: BrainCircuit },
      ...
      >
        ...
        <nav className={cn('space-y-6 pb-20', sidebarCollapsed ? 'p-2' : 'p-3')}>
          ...
          <div className="mt-4 pt-4 border-t border-slate-800/50">
            {(() => {
              const isArchiveActive = normalizedPathname === '/knowledge/archived' || normalizedPathname?.startsWith('/knowledge/archived/');
              return (
                <Link
                  href="/knowledge/archived"
                  aria-current={isArchiveActive ? 'page' : undefined}
                  title={sidebarCollapsed ? '归档' : undefined}
                  className={cn(
                ...
                >
                  <Archive className={cn('h-3.5 w-3.5', isArchiveActive ? 'text-slate-950' : 'text-slate-600')} />
                  {!sidebarCollapsed && <span className="text-xs font-medium">归档</span>}
                </Link>
              );
            })()}
          </div>
        ...
        <div className={cn('absolute bottom-0 left-0 right-0 border-t border-slate-800 bg-slate-950/95 backdrop-blur', sidebarCollapsed ? 'p-2' : 'p-3 space-y-2')}>
          {!sidebarCollapsed && <ThemeToggle className="w-full justify-center" />}
