...
export function BatchActionBar({ selectedIds, onClear, onDeleted }: BatchActionBarProps) {
  ...
  async function executeDelete() {
    ...
    const results = await Promise.all(
      ids.map((id) =>
        apiFetch(`/api/v1/knowledge/${id}/status`, {
          method: 'DELETE',
          body: JSON.stringify({ requestId: crypto.randomUUID() }),
        })
          .then(() => ({ ok: true as const, message: '' }))
          .catch((error) => ({
            ok: false as const,
    ...
    if (failed === 0) {
      ...
      toast.error(firstError ? `删除完成：${success} 条成功，${failed} 条失败。${firstError}` : `删除完成：${success} 条成功，${failed} 条失败`);
    }
