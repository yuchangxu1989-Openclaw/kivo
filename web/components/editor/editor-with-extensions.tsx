...
export function EditorWithExtensions({ value, onChange, height = 400, colorMode }: EditorWithExtensionsProps) {
  ...
  return (
    <div ref={containerRef} className="relative" data-color-mode={colorMode}>
      ...
      {wikiOpen && (
        <div className="absolute z-50 w-72 rounded-xl border border-slate-200 bg-white shadow-lg"
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            {filteredEntries.length === 0 ? (
              ...
              <button key={entry.id}
                className={`w-full rounded-lg px-3 py-2 text-left text-sm transition-colors ${i === wikiIdx ? 'bg-violet-50 text-violet-900' : 'hover:bg-slate-50'}`}
      ...
      {slashOpen && (
        <div className="absolute z-50 w-56 rounded-xl border border-slate-200 bg-white shadow-lg"
          ...
          <div className="max-h-72 overflow-y-auto p-1">
            {filteredCmds.length === 0 ? (
              ...
              <button key={cmd.label}
                className={`flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm transition-colors ${i === slashIdx ? 'bg-violet-50 text-violet-900' : 'hover:bg-slate-50'}`}
                onMouseDown={(e) => { e.preventDefault(); selectSlash(cmd); }}
                onMouseEnter={() => setSlashIdx(i)}>
                <span className="flex h-6 w-6 items-center justify-center rounded bg-slate-100 text-xs font-mono">{cmd.icon}</span>
                {cmd.label}
              </button>
            ))}
          </div>
