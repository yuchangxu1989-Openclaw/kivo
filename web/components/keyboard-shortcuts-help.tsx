...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="inline-flex h-6 min-w-6 items-center justify-center rounded-md border border-slate-200 bg-slate-50 px-1.5 font-mono text-xs font-medium text-slate-600">
      {children}
    </kbd>
  );
}
      ...
      <DialogPrimitive.Portal>
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <DialogPrimitive.Close className="rounded-md p-1 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600" aria-label="关闭">
              <X className="h-4 w-4" />
                ...
                <ul className="space-y-1.5">
                  {group.items.map((item) => (
                    <li
                      ...
                      className="flex items-center justify-between rounded-lg px-2 py-1.5 text-sm transition-colors hover:bg-slate-50"
                    >
