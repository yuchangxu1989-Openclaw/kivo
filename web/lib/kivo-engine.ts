...
function createConfig(dbPath: string): KivoConfig {
  return {
    dbPath,
...
export async function getKivo(): Promise<Kivo> {
  ...
  if (!instance || instancePath !== dbPath) {
    instance = new Kivo(createConfig(dbPath));
    instancePath = dbPath;
    seeded = false;
    await instance.init();
    if (!seeded) {
...
async function seedDemoData(): Promise<void> {
  const dbPath = resolveDbPath();
  const seedRepo = new KnowledgeRepository(new SQLiteProvider({ dbPath, configDir: process.cwd() }));
