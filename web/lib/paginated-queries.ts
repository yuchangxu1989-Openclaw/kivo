...
const KNOWLEDGE_TYPE_VALUES = ['fact', 'methodology', 'decision', 'experience', 'intent', 'meta'];
...
export function getEntryCounts(): {
  ...
  const d = getDb();
  const placeholders = WIKI_ENTRY_TYPES.map(() => '?').join(', ');
  const totalRow = d.prepare(`SELECT COUNT(*) as cnt FROM entries WHERE type NOT IN (${placeholders})`).get(...WIKI_ENTRY_TYPES) as { cnt: number };
  const typeRows = d.prepare(`SELECT type, COUNT(*) as cnt FROM entries WHERE type NOT IN (${placeholders}) GROUP BY type`).all(...WIKI_ENTRY_TYPES) as Array<{ type: string; cnt: number }>;
  const statusRows = d.prepare(`SELECT status, COUNT(*) as cnt FROM entries WHERE type NOT IN (${placeholders}) GROUP BY status`).all(...WIKI_ENTRY_TYPES) as Array<{ status: string; cnt: number }>;
  const lastRow = d.prepare(`SELECT MAX(updated_at) as last_updated FROM entries WHERE type NOT IN (${placeholders})`).get(...WIKI_ENTRY_TYPES) as { last_updated: string | null };
...
export function findEntriesByIds(ids: string[]): KnowledgeEntry[] {
  ...
  const sql = `SELECT * FROM entries WHERE id IN (${placeholders}) AND status = 'active'`;
...
/**
 * Count total entries and demo seed entries. Used by fresh-install status.
 */
