...
export function upsertIntent(input: { id?: string; name: string; description: string; positives: string[]; negatives: string[]; relatedEntryCount: number }) {
  ...
  if (input.id) {
    ...
    appendActivityEvent({ type: 'intent_updated', label: '意图更新', summary: `意图「${input.name}」已保存,并触发增量更新。`, href: '/governance', tags: ['governance', 'intent'] });
  ...
  appendActivityEvent({ type: 'intent_created', label: '意图新增', summary: `意图库新增意图「${item.name}」,增量更新已触发。`, href: '/governance', tags: ['governance', 'intent'] });
  return getIntentData();
}
...
export function deleteIntent(id: string) {
  ...
  if (target && before !== s.intents.length) appendActivityEvent({ type: 'intent_deleted', label: '意图删除', summary: `意图「${target.name}」已删除,关联条目数为 ${target.relatedEntryCount}。`, href: '/governance', tags: ['governance', 'intent'] });
