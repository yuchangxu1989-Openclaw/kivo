...
| `KIVO_EMBEDDING_TIMEOUT_MS` | 向量化请求超时（毫秒）。Ollama 第一次拉模型常用 30s | `10000` |
| `KIVO_INTERNAL_TOKEN` | 内部 cron / dispatcher 鉴权 token（FR-A02 多模态管线） | — |
| `AUTH_PASSWORD` | Web 工作台登录密码 | — |

Embedding provider 是必须的。未配置时命令会报错并输出配置引导，推荐 Ollama + bge-m3（本地免费）。`kivo init` 会自动检测并引导你完成配置。

### FR-A02 多模态管线 dispatcher（陌生人 90 秒上手）

陌生人上传的多模态素材（PDF / 图片 / 音频 / 视频）走 `materials` 表 → `task_queue` → 后台 dispatcher 推进到 slice → extract → wiki_page。dispatcher 每分钟由 cron 触发：

```bash
# 1) 生成 16 字节随机 token，写入 web/.env.local（首次安装 kivo init 已生成）
node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"

# 2) 把 token 同步到 systemd（让 web 进程重启也能拿到 token）
mkdir -p ~/.config/systemd/user/kivo-web.service.d
cat > ~/.config/systemd/user/kivo-web.service.d/internal-token.conf <<EOF
[Service]
Environment=KIVO_INTERNAL_TOKEN=<上一步生成的 token>
Environment=KIVO_EMBEDDING_TIMEOUT_MS=30000
EOF
systemctl --user daemon-reload && systemctl --user restart kivo-web

# 3) 把 dispatcher cron 写进 crontab（每分钟一次）
echo '* * * * * cd '"$(pwd)"' && bash scripts/cron-pipeline-dispatcher.sh >> logs/cron-pipeline-dispatcher.log 2>&1' \
  | (crontab -l 2>/dev/null; cat) | sort -u | crontab -

# 4) 自检
curl -sH "Authorization: Bearer $KIVO_INTERNAL_TOKEN" -X POST \
  http://127.0.0.1:3721/kivo/api/internal/dispatcher/tick \
  -d '{"concurrency":3}' | jq '.dispatched, .succeeded, .failed'
```

期望输出 `dispatched=N, succeeded≈N, failed=0`。失败 401 → 说明 web 进程没读到 token（看 `systemctl --user show kivo-web -p Environment | grep KIVO_INTERNAL_TOKEN`）。
...
- Ollama + bge-m3（或其他兼容的 embedding provider）—— FR-A02 dispatcher 分类阶段的硬依赖
- OpenClaw 宿主环境
