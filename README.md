...
- 分析产物审核——调研产出的候选知识逐条审核，确认/拒绝/编辑后入库，不让垃圾混进知识库
- 意图治理面板——高频主题柱状图、治理报告历史、批量归档/重评，知识库健康状况一目了然
...
知识库里 3 个月没人碰的条目，是还有用还是已经过时了？KIVO 用 LLM 逐条判断：如果知识描述的技术、规则、决策已经被新版本替代或不再适用，自动归档。不删除，但不再参与检索和注入，避免过时知识误导 Agent。每周日凌晨自动执行，零人工干预。
...
手动跑去重、审计、重写？太累了。`kivo auto-govern` 一条命令串联 MECE 去重 + 质量审计 + 低分条目自动重写，挂到 cron 每天凌晨跑一次，知识库自己保持健康。`kivo watch-badcases` 监听指定目录，新 badcase 文件落盘即自动提取意图知识。`kivo init` 在 OpenClaw 环境下自动注册 crontab + 安装意图注入 hook，装完即自治，零人工干预。`kivo embed-backfill` 对存量知识分批向量化（每批 20 条，自动跑完全部未向量化条目），`kivo governance check-staleness` 检测 90 天未更新的条目并用 LLM 判断是否过时，过时条目自动归档。两个命令都已注册到 cron，装完即自动运行。
