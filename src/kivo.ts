...
import { resolveEmbeddingConfig } from './embedding/resolve-embedding-config.js';
...
export class Kivo {
  ...
  async init(): Promise<void> {
    ...
    this.pipeline = new PipelineEngine({
      ...
      domainGoals: () => this.domainGoalStore?.list() ?? [],
    });
    ...
    if (this.config.embedding) {
      const cfg = resolveEmbeddingConfig(this.config.embedding);
      ...
      if (cfg.provider === 'openai-compatible') {
        ...
        baseProvider = new LocalEmbedding(cfg.dimensions);
      } else {
        // ollama — use OpenAIEmbedding with Ollama's compatible endpoint
        baseProvider = new OpenAIEmbedding({
          apiKey: 'ollama',
      ...
      this.embeddingCache = new EmbeddingCache(baseProvider, 1000);
  ...
  async ingest(text: string, source: string): Promise<IngestResult> {
    ...
    for (const entry of entries) {
      ...
      // Persist entry
      const constraintDecision = await enforceConstraints(entry, this.domainGoalStore.list());
  ...
  async resolveConflict(
    ...
    // Apply resolution: mark loser as superseded (soft-delete via DB status column)
    if (result.action === 'supersede') {
      await this.repository.updateStatus(result.loserId, 'active');
    }
  ...
  /** 获取未处理的规则分发告警 */
  getUnhandledDistributionAlerts() {
    this.ensureInitialized();
    return this.ruleDistributor.getUnhandledAlerts();
  }
