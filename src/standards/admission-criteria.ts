...
export const HUMAN_READABLE_INTENT_STYLE_PROMPT = `### 人话意图写作标准
- title 必须是口语化、可理解的人话标题，像跟同事解释时会自然说出的一句话，不是学术摘要、标签名或压缩后的分类名。
- title 必须像用户本人会说的话，10-25字，具体说清「谁/什么场景下，对什么对象做什么」。优先使用「具体动词 + 具体对象」，例如「禁止用正则做语义判断」「修问题先堵根因再处理症状」。
- 标题必须能让人一眼知道要采取什么行动、避免什么错误或记住什么边界；可以使用必要技术名词（如 LLM、向量检索、流水线、prompt），但不能让技术词单独当标题。
- 禁止 2-6 字的抽象名词短语，禁止缩略语、省略动作、四字成语式标题和 AI 摘要腔；避免「策略」「原则」「机制」「规范」「优化」「对齐」「治理」「框架」等抽象名词堆砌，除非标题里同时有明确动作和对象。
- 如果初稿像「语义禁用规则」「知识对齐策略」「领域知识库通用化」这种抽象短标题，必须改写成一句人会说的话，例如「禁止用正则做语义判断」「知识库内容要跟 spec 保持一致」「知识库不要绑死在单一领域」。
- 好标题："禁止用正则做语义判断"、"知识库内容要跟 spec 保持一致"、"知识库不要绑死在单一领域"、"提取知识时标题要像人说话"、"用按钮让用户直接进入详情"。
- 坏标题："语义禁用规则"、"知识对齐策略"、"领域知识库通用化"、"标题优化"、"抽象边界"。
- content/description 必须有具体场景：用一两句话说清「什么触发场景下、该做什么、不做会造成什么后果」。禁止抽象概括，禁止和 title 高度重复。
- why 必须填写独立记录理由：一句话说清不这样做的后果、踩坑代价或失败模式。why 禁止复制 content/description/summary/title，禁止留空、"待补充"或改写复述；如果无法从文本可靠推断 why，丢弃这条候选，不要输出。
- similar_sentences 必须生成 2-3 条泛化相似表述，用于后续语义检索匹配，禁止复制原句。`;

export function buildHumanReadableIntentStyleSection(): string {
  return HUMAN_READABLE_INTENT_STYLE_PROMPT;
}
