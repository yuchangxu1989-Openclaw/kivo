...
export class ConsistencyChecker {
  ...
  check(entries: KnowledgeEntry[], options: ConsistencyCheckOptions = {}): ConsistencyReport {
    ...
    // 2. Stale reference detection: active entries referencing superseded/archived/deprecated entries
    const entryMap = new Map(allEntries.map(e => [e.id, e]));
    for (const entry of activeEntries) {
      if (entry.supersedes) {
        const target = entryMap.get(entry.supersedes);
        if (target && target.status === 'active') {
          ...
          issues.push({
            ...
            entryIdB: target.id,
            titleB: target.title,
            description: `Entry "${entry.title}" supersedes "${target.title}" but the superseded entry is still active`,
          });
        }
      }
      ...
      if (entry.dependencies) {
        for (const dep of entry.dependencies) {
          ...
          if (target && (target.status === 'archived' || target.status === 'deprecated' || target.status === 'superseded')) {
            issueCounter++;
            issues.push({
              id: `consistency-${issueCounter}`,
              ...
              entryIdB: target.id,
              titleB: target.title,
              description: `Entry "${entry.title}" ${dep.relation} "${target.title}" which is ${target.status}`,
            });
          }
        }
      }
