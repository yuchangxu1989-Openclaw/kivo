...
export interface IntentGovernanceEngineOptions extends OperationalDbOptions {
...
export class IntentGovernanceEngine {
  ...
  constructor(store: GovernanceStore, config?: Partial<GovernanceConfig> | IntentGovernanceEngineOptions) {
    this.store = store;
    const engineOptions = normalizeEngineOptions(config);
    this.config = { ...DEFAULT_GOVERNANCE_CONFIG, ...engineOptions.config };
    this.dbOptions = { cwd: engineOptions.cwd, dbPath: engineOptions.dbPath };
  }
  ...
  async runGovernance(): Promise<GovernanceReport> {
    ...
    const now = new Date();
    const originalIntentSnapshots = intents.map((intent) => ({ ...intent }));
    ...
    for (const cluster of clusters) {
      ...
      this.persistGovernanceSnapshot(op.id, 'merge', activeMembers, { resultId: created.id });
    }
    ...
    if (weightChanges.length > 0) {
      const changedIds = new Set(weightChanges.map((change) => change.intentId));
      this.persistGovernanceSnapshot(
        `weights-${Date.now()}`,
        'weight_adjustment',
        originalIntentSnapshots.filter((intent) => changedIds.has(intent.id)),
  ...
  async overrideWeight(intentId: string, newWeight: number): Promise<void> {
    ...
    if (!intent) return;
    this.persistGovernanceSnapshot(`weight-${intent.id}-${Date.now()}`, 'weight_adjustment', [intent]);
  ...
  async rollbackGovernance(operationId: string): Promise<boolean> {
    const db = openOperationalDb(this.dbOptions);
    try {
      ensureOperationalTables(db);
      const row = db.prepare(`
        SELECT operation_type, before_state_json, after_state_json
      ...
      const beforeState = JSON.parse(row.before_state_json) as GovernableIntent[];
      for (const snapshot of beforeState) {
        await this.store.update(reviveIntent(snapshot));
      }

      if (row.operation_type === 'merge' && row.after_state_json) {
        const afterState = JSON.parse(row.after_state_json) as { resultId?: string };
        if (afterState.resultId) {
          const merged = (await this.store.listActive()).find((intent) => intent.id === afterState.resultId);
          if (merged) {
            merged.governanceStatus = 'merged';
            await this.store.update(merged);
          }
  ...
  private persistGovernanceSnapshot(
    operationId: string,
    operationType: string,
    beforeState: GovernableIntent[],
    ...
    try {
      ensureOperationalTables(db);
      db.prepare(`
        INSERT INTO governance_snapshots (operation_id, operation_type, before_state_json, after_state_json, created_at)
        VALUES (?, ?, ?, ?, ?)
...
function normalizeEngineOptions(
  config?: Partial<GovernanceConfig> | IntentGovernanceEngineOptions,
): IntentGovernanceEngineOptions {
  if (!config) {
    return {};
  }

  if ('config' in config || 'cwd' in config || 'dbPath' in config) {
    return config as IntentGovernanceEngineOptions;
  }

  return { config: config as Partial<GovernanceConfig> };
}
