...
describe('FR-E06 AC7: governance rollback', () => {
  it('rolls back merge operations from persisted snapshot', async () => {
    const dbPath = join(process.cwd(), 'tmp', 'intent-governance-test.db');
    rmSync(dbPath, { force: true });
    const store = createMockStore();
    store.intents.push(
      makeIntent({ id: 'm1', name: '中文注释', description: '代码注释用中文', positives: ['中文注释代码'] }),
      makeIntent({ id: 'm2', name: '中文注释规范', description: '代码注释用中文', positives: ['中文注释代码'] }),
    );

    const engine = new IntentGovernanceEngine(store, {
      dbPath,
      config: { similarityThreshold: 0.3, highFrequencyMinCount: 100 },
    });

    const report = await engine.runGovernance();
    const operationId = report.mergeOperations[0]?.id;
    expect(operationId).toBeTruthy();

    const rolledBack = await engine.rollbackGovernance(operationId!);
    expect(rolledBack).toBe(true);
    expect(store.intents.find((intent) => intent.id === 'm1')?.governanceStatus).toBe('active');
    expect(store.intents.find((intent) => intent.id === 'm2')?.governanceStatus).toBe('active');
  });
});
