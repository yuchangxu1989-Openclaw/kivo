...
export type EntryStatus = 'active' | 'pending_review' | 'rejected';
...
export interface KnowledgeEntry {
  ...
  /** Positive examples for intent-type entries (FR-E04) */
  positiveExamples?: string[];
  /** Negative examples for intent-type entries (FR-E04) */
...
export interface KnowledgeSource {
  ...
  /** Source position — line number, paragraph index, or section path for traceability (FR-A02) */
  position?: SourcePosition;
}

/** Precise location within a source document (FR-A02) */
export interface SourcePosition {
  /** Line number (1-based) within the source document */
  line?: number;
  /** Paragraph index (0-based) */
  paragraph?: number;
  /** Section heading path, e.g. "Chapter 1 > Introduction" */
  section?: string;
}
...
// ─── Intent Knowledge (FR-E04) ───────────────────────────────────────────────

/**
 * IntentKnowledge — specialized KnowledgeEntry for intent-type entries.
 * Requires at least one of positiveExamples or negativeExamples to be non-empty.
 */
export interface IntentKnowledge extends KnowledgeEntry {
  type: 'intent';
  /** Positive examples: utterances/patterns that match this intent */
  positiveExamples: string[];
  /** Negative examples: utterances/patterns that should NOT match this intent */
  negativeExamples: string[];
}

/** Type guard: checks if a KnowledgeEntry is a valid IntentKnowledge */
export function isIntentKnowledge(entry: KnowledgeEntry): entry is IntentKnowledge {
  return (
    entry.type === 'intent' &&
    Array.isArray((entry as IntentKnowledge).positiveExamples) &&
    Array.isArray((entry as IntentKnowledge).negativeExamples) &&
    ((entry as IntentKnowledge).positiveExamples.length > 0 ||
      (entry as IntentKnowledge).negativeExamples.length > 0)
  );
}
