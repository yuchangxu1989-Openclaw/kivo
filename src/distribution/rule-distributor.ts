...
export class RuleDistributor {
  ...
  private async finalizeDistribution(
    ...
    if (unresolvedIds.length > 0) {
      this.persistAlert(event.ruleId, retryFailures.remainingFailures, distributedAt);
    }
  ...
  private async retryFailures(
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries && pendingFailures.length > 0; attempt += 1) {
      const backoffMs = this.config.retryDelayMs > 0 ? this.config.retryDelayMs * (2 ** (attempt - 1)) : 0;
      if (backoffMs > 0) {
  ...
  getUnhandledAlerts(): DistributionAlert[] {
    const db = openOperationalDb(this.dbOptions);
    try {
      ensureOperationalTables(db);
      const rows = db.prepare(`
        SELECT id, rule_id, error, timestamp, handled, metadata_json
        FROM distribution_alerts
        WHERE handled = 0
  ...
  private persistAlert(ruleId: string, failures: RetryableFailure[], timestamp: Date): void {
    const db = openOperationalDb(this.dbOptions);
    try {
      ensureOperationalTables(db);
      const error = failures
        .map((failure) => `${failure.subscriptionId}: ${String(failure.error instanceof Error ? failure.error.message : failure.error)}`)
        .join(' | ');
      db.prepare(`
        INSERT INTO distribution_alerts (id, rule_id, error, timestamp, handled, metadata_json)
        VALUES (?, ?, ?, ?, 0, ?)
      `).run(
        randomUUID(),
        ruleId,
        ...
        JSON.stringify({
          failedSubscriptionIds: failures.map((failure) => failure.subscriptionId),
        }),
      );
    } finally {
      db.close();
