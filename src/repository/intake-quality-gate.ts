...
const BGE_M3_BLOB_SIZE = BGE_M3_DIMS * 4;
...
export class IntakeQualityGate {
  ...
  constructor(options: IntakeQualityGateOptions) {
    ...
    this.embedder = new OllamaEmbeddingProvider({ model: 'bge-m3' });
    this.ensureSchema();
  }
  ...
  async close(): Promise<void> {
    // OllamaEmbeddingProvider uses stateless HTTP requests; no persistent handle.
  }
    ...
    const rows = this.db.prepare(
      `SELECT id, title, embedding
       ...
       WHERE status = 'active'
         ...
         AND (? IS NULL OR id != ?)`
    ).all(BGE_M3_BLOB_SIZE, ignoreEntryId ?? null, ignoreEntryId ?? null) as ExistingEmbeddingRow[];
