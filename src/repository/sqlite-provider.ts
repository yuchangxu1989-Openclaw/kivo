...
export class SQLiteProvider implements StorageProvider {
  ...
  private async checkSemanticDuplicate(entry: KnowledgeEntry): Promise<{ duplicateId: string; existingConfidence: number } | null> {
    try {
      ...
      const rows = this.db.prepare(
        `SELECT id, title, content, confidence, embedding FROM entries WHERE status = 'active' AND embedding IS NOT NULL`
  ...
  private async vectorSearch(text: string, conditions: string[], params: unknown[], limit: number): Promise<SearchResult[] | null> {
    try {
      ...
      const embCount = (this.db.prepare('SELECT COUNT(*) as cnt FROM entries WHERE embedding IS NOT NULL').get() as { cnt: number }).cnt;
      ...
      const sql = `SELECT e.*, e.embedding as _embedding FROM entries e WHERE e.embedding IS NOT NULL${filterClause}`;
