...
// ─── FR-FIX-09: Dynamic Threshold & P75 Weight ───────────────────────────────
...
const thresholdCache = new Map<string, ThresholdCache>();
const p75Cache = new Map<string, P75Cache>();

/**
 * FR-FIX-09 AC1: Compute dynamic association threshold based on graph density.
 * Density = edges / (nodes * (nodes - 1) / 2) for undirected graph.
 * Higher density → higher threshold (more selective).
 * Lower density → lower threshold (more inclusive).
 */
export function computeDynamicThreshold(snapshot: KnowledgeGraphSnapshot): number {
  const cacheKey = createSnapshotCacheKey(snapshot);
  const now = Date.now();
  ...
  if (nodeCount < 2) {
    thresholdCache.set(cacheKey, { value: 0.3, computedAt: now });
    return 0.3;
  }
  ...
  // Map density to threshold: sparse graphs (density < 0.05) → 0.2, dense (> 0.3) → 0.6
  // Linear interpolation between bounds
  const minThreshold = 0.2;
  const maxThreshold = 0.6;
  const normalizedDensity = Math.min(1, Math.max(0, (density - 0.01) / 0.3));
  const threshold = minThreshold + normalizedDensity * (maxThreshold - minThreshold);

  thresholdCache.set(cacheKey, { value: threshold, computedAt: now });
  return threshold;
}

/**
 * FR-FIX-09 AC3: Compute P75 (75th percentile) of all edge weights.
 * Edges below P75 are down-weighted during injection.
 */
export function computeP75Weight(snapshot: KnowledgeGraphSnapshot): number {
  const cacheKey = createSnapshotCacheKey(snapshot);
  const now = Date.now();
  const cached = p75Cache.get(cacheKey);
  if (cached && (now - cached.computedAt) < CACHE_TTL_MS) {
    return cached.value;
  }

  const weights = snapshot.edges.map(e => e.strength).sort((a, b) => a - b);

  if (weights.length === 0) {
    p75Cache.set(cacheKey, { value: 0.5, computedAt: now });
    return 0.5;
  }

  // P75 index
  const idx = Math.floor(weights.length * 0.75);
  const p75 = weights[Math.min(idx, weights.length - 1)];

  p75Cache.set(cacheKey, { value: p75, computedAt: now });
  return p75;
}

/**
 * FR-FIX-09 AC4: Apply weight annotation to injection results.
 * Entries associated via edges below P75 get a weight penalty.
 */
export interface WeightAnnotatedResult {
  entryId: string;
  weight: number;
  belowP75: boolean;
}

export function annotateWeights(
  edgeWeights: Array<{ entryId: string; weight: number }>,
  p75: number,
): WeightAnnotatedResult[] {
  return edgeWeights.map(({ entryId, weight }) => ({
    entryId,
    weight: weight < p75 ? weight * 0.5 : weight,
    belowP75: weight < p75,
  }));
}

/** Clear threshold/P75 caches (for testing or forced recomputation). */
export function clearInsightCaches(): void {
  thresholdCache.clear();
  p75Cache.clear();
}

function createSnapshotCacheKey(snapshot: KnowledgeGraphSnapshot): string {
  return `${snapshot.nodes.length}:${snapshot.edges.length}:${snapshot.edges.map((edge) => edge.id).sort().join('|')}`;
}
