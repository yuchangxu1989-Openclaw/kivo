...
import { cosineSimilarity } from './association-discovery.js';
...
export class KnowledgeGraphBuilder {
  ...
  async incrementalUpdate(
    ...
    // Semantic proximity for changed entries (using cosine similarity on embeddings)
    const embeddings = options.embeddings;
    if (embeddings && embeddings.size >= 2) {
      for (const changed of changedEntries) {
        const vecChanged = embeddings.get(changed.id);
        if (!vecChanged) continue;

        for (const other of fullEntries) {
          if (changed.id === other.id) continue;
          const vecOther = embeddings.get(other.id);
          if (!vecOther) continue;

          const similarity = cosineSimilarity(vecChanged, vecOther);
...
export function buildSnapshot(
  ...
  if (embeddings && embeddings.size >= 2) {
    forEachPair(entries, (a, b) => {
      ...
      if (similarity < semanticThreshold) return;
...
// lexicalOverlap and tokenize removed — replaced by cosineSimilarity from association-discovery.ts
