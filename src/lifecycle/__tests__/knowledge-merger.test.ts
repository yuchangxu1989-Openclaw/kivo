...
describe('KnowledgeMerger', () => {
  ...
  beforeEach(() => {
    rmSync(join(process.cwd(), 'tmp', 'knowledge-merger-test.db'), { force: true });
    store = new MemoryKnowledgeStore();
    merger = new KnowledgeMerger({
      store,
      now: () => new Date('2026-04-20T12:00:00.000Z'),
      idGenerator: () => 'merged-entry-1',
      dbPath: join(process.cwd(), 'tmp', 'knowledge-merger-test.db'),
    });
  });
  ...
  it('merges entries and preserves all source references', async () => {
    ...
    expect(originalOne?.status).toBe('active');
    expect(originalTwo?.status).toBe('active');
  });
  ...
  it('supports rollbackMerge alias with persisted snapshot', async () => {
    await store.save(makeEntry({ id: 'entry-1', content: 'original one' }));
    await store.save(makeEntry({ id: 'entry-2', content: 'original two', source: testSource('doc://other') }));
    ...
    const restored = await merger.rollbackMerge('merged-entry-1');
    expect(restored.restoredEntryIds).toEqual(['entry-1', 'entry-2']);
    expect((await store.get('entry-1'))?.content).toBe('original one');
  });
