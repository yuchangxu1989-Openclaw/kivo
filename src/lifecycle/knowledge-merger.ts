...
export class KnowledgeMerger {
  ...
  async merge(candidate: MergeCandidate): Promise<MergedEntry> {
    ...
    // FR-C03 AC2: methodology 类型合并后仍为 active
    mergedEntry.status = 'active';

    const snapshot = {
      mergedEntry,
      originalEntries: entries,
    };
    this.snapshots.set(mergedEntry.id, snapshot);
    this.persistSnapshot(mergedEntry.id, snapshot, timestamp);
  ...
  async revert(mergedEntryId: string): Promise<MergeReversal> {
    const snapshot = this.snapshots.get(mergedEntryId) ?? this.loadSnapshot(mergedEntryId);
    if (!snapshot) {
  ...
  async rollbackMerge(mergeId: string): Promise<MergeReversal> {
    return this.revert(mergeId);
  }
  ...
  private persistSnapshot(mergeId: string, snapshot: MergeSnapshot, createdAt: Date): void {
    const db = openOperationalDb(this.dbOptions);
    try {
      ensureOperationalTables(db);
      db.prepare(`
        INSERT INTO merge_snapshots (merge_id, merged_entry_json, original_entries_json, created_at)
        VALUES (?, ?, ?, ?)
        ON CONFLICT(merge_id) DO UPDATE SET
          merged_entry_json = excluded.merged_entry_json,
          original_entries_json = excluded.original_entries_json,
          created_at = excluded.created_at
      `).run(
        mergeId,
        JSON.stringify(snapshot.mergedEntry),
        JSON.stringify(snapshot.originalEntries),
  ...
  private loadSnapshot(mergeId: string): MergeSnapshot | null {
    const db = openOperationalDb(this.dbOptions);
    try {
      ensureOperationalTables(db);
      const row = db.prepare(`
        SELECT merged_entry_json, original_entries_json
        FROM merge_snapshots
        WHERE merge_id = ?
      `).get(mergeId) as { merged_entry_json: string; original_entries_json: string } | undefined;
      ...
      return {
        mergedEntry: reviveEntry(JSON.parse(row.merged_entry_json)) as MergedEntry,
        originalEntries: (JSON.parse(row.original_entries_json) as unknown[]).map((entry) => reviveEntry(entry)),
      };
    } finally {
      db.close();
    }
...
function reviveEntry(entry: unknown): KnowledgeEntry {
  const value = entry as KnowledgeEntry & { sourceRefs?: SourceRef[]; mergedAt?: string | Date };
  return {
    ...value,
    source: {
      ...value.source,
      timestamp: new Date(value.source.timestamp),
    },
    ...
    mergedAt: value.mergedAt ? new Date(value.mergedAt) : undefined,
  } as KnowledgeEntry;
}
