...
export interface DeduplicateReport {
  ...
  autoMerged: number;
  fragmentAggregation?: AggregationReport;
  domain?: string;
}
 ...
 * Returns the best match if similarity > threshold (from kivo.config.json).
 */
export function checkPreIngestDuplicate(
  newVector: number[],
  dbPath?: string,
  threshold: number = loadDedupThreshold(),
...
export interface DeduplicateOptions {
  /** Similarity threshold for flagging duplicates (reads from kivo.config.json by default) */
  threshold?: number;
  /** Auto-merge entries with similarity > 0.95 and run LLM fragment aggregation */
  auto?: boolean;
...
export async function runDeduplicateScan(
  ...
  const { threshold = loadDedupThreshold(), auto = false, domain } = options;
  ...
  try {
      ...
      for (const suggestion of mergeSuggestions) {
        if (suggestion.pair.similarity > autoMergeThreshold) {
          ...
          if (removedRow) {
            ...
            // Soft-delete the duplicate (status stays 'active'; merge log records the action)
            db.prepare('UPDATE entries SET status = \'active\' WHERE id = ?').run(suggestion.removeId);
            autoMerged++;
          }
        }
      }
    ...
    let fragmentAggregation: AggregationReport | undefined;

    // FR-N09: LLM-driven fragment aggregation. Vector similarity only forms
    // candidate groups; the actual merge decision/content is produced by LLM
    // inside FragmentAggregator, preserving source entry references.
    if (auto) {
      const provider = new SQLiteProvider({ dbPath: resolvedDb, configDir: process.cwd() });
      try {
        const repository = new KnowledgeRepository(provider);
        const aggregator = new FragmentAggregator(repository);
        fragmentAggregation = await aggregator.run({
...
export async function checkDuplicate(
  ...
  try {
    ...
    const result = checkPreIngestDuplicate(vector, resolvedDb);
    return {
...
export function formatDeduplicateReport(report: DeduplicateReport): string {
  ...
  if (report.mergeSuggestions.length === 0) {
    ...
    appendFragmentAggregationLines(lines, report);
    return lines.join('\n');
  }
...
function appendFragmentAggregationLines(lines: string[], report: DeduplicateReport): void {
  if (!report.fragmentAggregation) return;
  lines.push('');
  lines.push('碎片聚合（FR-N09）:');
  lines.push(`  发现分组: ${report.fragmentAggregation.groupsDetected}`);
  lines.push(`  合并条目: ${report.fragmentAggregation.entriesMerged}`);
  lines.push(`  原条目标记 superseded: ${report.fragmentAggregation.entriesRemoved}`);
  const aggregationErrors = (report.fragmentAggregation as unknown as { errors?: string[] }).errors ?? [];
  if (aggregationErrors.length > 0) {
    lines.push(`  错误: ${aggregationErrors.slice(0, 3).join('; ')}`);
  }
}
