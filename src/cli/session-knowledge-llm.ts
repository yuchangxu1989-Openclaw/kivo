...
function buildSessionExtractionPrompt(segments: Segment[]): string {
  ...
  return `从以下用户与 AI 助手的对话片段中提取有价值的知识条目。
...
过滤掉：
- 纯操作性内容（"帮我打开文件"、"运行这个命令"）
- 临时性对话（闲聊、确认收到）
- 重复内容
- 配置片段（JSON/YAML 配置、环境变量设置、命令行参数）
- 系统提示词/铁律注入内容（这些是系统上下文，不是用户知识）
...
1. nature（知识本质）: fact / concept / rule / procedure / heuristic
   ...
   - rule: 明确规则、约束条件
   - procedure: 步骤流程、操作方法
   - heuristic: 经验法则、启发式判断
2. function（知识用途）: routing / quality_gate / context_enrichment / decision_support / correction
   - routing: 意图路由、任务分发
   - quality_gate: 质量把关、审计规则
   - context_enrichment: 上下文补充、背景知识
   - decision_support: 决策支持、权衡分析
   - correction: 纠偏、错误修正
3. domain（知识领域）: 开放标签，如 "agent-scheduling"、"product-design"、"code-quality"

输出纯 JSON 数组，每条格式：
{"content":"知识内容","title":"简短标题（≤50字符）","nature":"<nature>","function":"<function>","domain":"<domain>","source":"session","confidence":0.0-1.0,"tags":["标签"]}

中文输入产出中文知识。如果没有可提取的知识，返回空数组 []。
  ...
  for (const cluster of clusters) {
    ...
    try {
      ...
      for (const item of items) {
        if (!item.content || item.content.trim().length < 10) continue;
