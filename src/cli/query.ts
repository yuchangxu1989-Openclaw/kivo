...
export async function runQuery(queryText: string, filters?: QueryFilterOptions): Promise<string> {
  ...
  // FR-B02: Add fallback hint when vector search was unavailable
  const fallbackHint = detectVectorSearchUnavailableReason(resolvedDb);
  if (fallbackHint && ftsResult.ids.length > 0) {
    return `\u26a0 ${fallbackHint}\n\n${ftsResult.text}`;
  }
 ...
 * Returns null if vector search should be working (meaning FTS fallback is just because no results matched).
 */
function detectVectorSearchUnavailableReason(dbPath: string): string | null {
  try {
