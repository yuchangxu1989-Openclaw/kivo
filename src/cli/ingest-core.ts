/**
 * Shared ingest core — LLM semantic extraction pipeline.
 *
 * Used by both `kivo ingest` and `kivo cron` to avoid code duplication.
 * Handles: chunk splitting, LLM extraction, entry storage.
 * BGE embedding is deferred to batch vectorization (embed-backfill).
 */
...
import type { KnowledgeSource, KnowledgeEntry, KnowledgeType, EntryStatus } from '../types/index.js';
...
export interface IngestCoreOptions {
  ...
  mdFiles: string[];
  /** Original source references for generated/intermediate markdown files */
...
## 知识准入
知识是经过抽象、聚合、萃取后形成的长效理解模型，必须跨时间、跨场景可复用，并能回答：它让 Agent 在什么场景下避免什么错误。
...
- methodology: 跨场景方法和原则
- experience: 抽象后的经验教训和踩坑模式
- fact/meta: 用户私有定义或知识系统规则
...
interface IngestSidecar {
  metadataBySection?: Record<string, Record<string, unknown>>;
  statusBySection?: Record<string, EntryStatus>;
  confidenceBySection?: Record<string, number>;
  typeBySection?: Record<string, KnowledgeType>;
  tagsBySection?: Record<string, string[]>;
}

function loadIngestSidecar(filePath: string): IngestSidecar {
  const sidecarPath = `${filePath}.kivo-metadata.json`;
  if (!existsSync(sidecarPath)) return {};
  try {
    return JSON.parse(readFileSync(sidecarPath, 'utf-8')) as IngestSidecar;
  } catch {
    return {};
  }
}
 ...
 *
 * Handles LLM extraction, entry storage, and proper cleanup.
 * BGE embedding is deferred to batch vectorization (embed-backfill).
 */
export async function runIngestCore(options: IngestCoreOptions): Promise<IngestCoreResult> {
  ...
  // BGE embedding deferred to batch vectorization — no embedder needed at ingest time
  console.log('Note: BGE embedding deferred to batch vectorization (kivo embed-backfill)');

  // Vector store no longer needed at ingest time (dedup deferred to batch vectorization)
  ...
  try {
    for (const filePath of mdFiles) {
      ...
      const sourceReference = options.sourceReferences?.[filePath] ?? `file://${relPath}`;
      const source: KnowledgeSource = {
        type: 'document',
        reference: sourceReference,
        timestamp: new Date(),
      };
      const sidecar = loadIngestSidecar(filePath);
      ...
      for (const chunk of chunks) {
        ...
        // Step 1: Skip BGE embedding for chunk-level vector dedup (deferred to batch vectorization)
        // Previously embedded each chunk and checked vector similarity; now skip to reduce latency.

        // Step 2: Vector dedup skipped (no chunk embedding available)
        ...
        const chunkSection = typeof chunk.metadata?.title === 'string' ? chunk.metadata.title : '';
        const sidecarMetadata = chunkSection ? sidecar.metadataBySection?.[chunkSection] : undefined;
        const sidecarStatus = chunkSection ? sidecar.statusBySection?.[chunkSection] : undefined;
        const sidecarConfidence = chunkSection ? sidecar.confidenceBySection?.[chunkSection] : undefined;
        const sidecarType = chunkSection ? sidecar.typeBySection?.[chunkSection] : undefined;
        const sidecarTags = chunkSection ? sidecar.tagsBySection?.[chunkSection] : undefined;
        ...
        for (const item of extracted) {
          ...
          const entry: KnowledgeEntry = {
            ...
            source,
            confidence: typeof sidecarConfidence === 'number' ? sidecarConfidence : 0.8,
            status: sidecarStatus ?? 'active',
            tags: [...(Array.isArray(item.tags) ? item.tags : []), ...(Array.isArray(sidecarTags) ? sidecarTags : [])],
          ...
          // FR-C01: Skip write-time conflict detection via embedding (deferred to batch vectorization)
          // Conflict detection will run during batch vectorization when embeddings are generated.

          // FR-N05 统一门禁在 repository.save 中执行；这里不再做局部门禁
          const saved = await repository.save(entry, { skipQualityGate: noQualityGate, skipEmbedding: true });
          if (!saved) {
            totalSkipped++;
            continue;
          }
