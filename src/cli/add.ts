...
export async function runAdd(type: string, title: string, options: AddOptions = {}): Promise<string> {
  ...
  try {
    const saved = await repository.save(entry, { skipQualityGate: !!options.noQualityGate });
    if (!saved) {
  ...
  if (options.json) {
    return JSON.stringify({ id, type, title: normalizedTitle, content, tags, confidence, status, domain, createdAt: now }, null, 2);
  }
