...
function logStalenessAction(
  ...
  `).run(
    ...
    'content_outdated',
    reasoning,
    JSON.stringify({ action: 'staleness_flagged', reasoning }),
    now,
  );
}
            ...
            if (item.is_stale) {
              ...
              if (!dryRun) {
                db.prepare('UPDATE entries SET status = ?, updated_at = ? WHERE id = ?')
                  .run('active', new Date().toISOString(), entry.id);
...
export function formatStalenessReport(report: StalenessReport): string {
  ...
  lines.push(`已标记: ${report.flaggedCount}`);
  lines.push('');
