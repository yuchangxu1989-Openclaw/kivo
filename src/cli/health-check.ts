...
export async function runHealthCheck(configPath?: string): Promise<HealthReport> {
  ...
  items.push(await checkEmbeddingProviderLive());
...
async function checkEmbeddingProviderLive(): Promise<HealthCheckItem> {
  try {
    const { checkEmbeddingHealth } = await import('../embedding/health-check.js');
    const result = await checkEmbeddingHealth();
    const dims = result.dimensions ? ` (${result.dimensions}-dim)` : '';
    return {
      name: 'Embedding',
      status: 'ok',
      detail: `✅ ${result.provider} ${result.model}${dims}`,
    };
  } catch (err) {
    const msg = (err as Error).message || 'Unknown error';
    // Distinguish between "not configured" and "unreachable"
    if (msg.includes('不可达') || msg.includes('Unreachable') || msg.includes('not running')) {
      return {
        name: 'Embedding',
        status: 'fail',
        detail: '❌ Ollama not running',
        suggestion: 'curl -fsSL https://ollama.com/install.sh | sh && ollama serve && ollama pull bge-m3',
      };
    }
    if (msg.includes('未加载') || msg.includes('Not Loaded') || msg.includes('pull')) {
      return {
        name: 'Embedding',
        status: 'warn',
        detail: '⚠ Ollama running but model not loaded',
        suggestion: 'ollama pull bge-m3',
      };
    }
    return {
      name: 'Embedding',
      status: 'warn',
      detail: `⚠ ${msg}`,
      suggestion: '运行 npx kivo init 配置向量检索，或安装 Ollama: curl -fsSL https://ollama.com/install.sh | sh',
    };
  }
}
