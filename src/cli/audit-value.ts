 ...
 * and reports high/low value breakdown. Optionally marks low-value entries as pending.
 */
...
export interface AuditValueOptions {
  ...
  limit?: number;
  /** Actually mark low-value entries as pending */
  apply?: boolean;
  /** Output as JSON */
  json?: boolean;
}
...
export async function runAuditValue(options: AuditValueOptions = {}): Promise<string> {
  ...
  // Optionally apply: mark low-value as pending
  if (options.apply && lowValue.length > 0) {
    const now = new Date().toISOString();
    const stmt = db.prepare('UPDATE entries SET status = ?, updated_at = ? WHERE id = ?');
    const txn = db.transaction(() => {
      for (const entry of lowValue) {
        stmt.run('pending', now, entry.id);
      }
    });
    txn();
    console.log(`Marked ${lowValue.length} low-value entries as pending.`);
  }
  ...
  if (options.apply) {
    lines.push(`✓ ${lowValue.length} low-value entries marked as pending.`);
  } else if (lowValue.length > 0) {
    lines.push(`Run with --apply to mark low-value entries as pending.`);
  }
