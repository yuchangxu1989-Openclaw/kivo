 ...
 *
 * AC coverage:
 *   AC4: --batch mode, old version preserved as superseded
...
export async function runUpgradeQuality(options: UpgradeQualityOptions = {}): Promise<string> {
  ...
  for (const assessment of failing) {
    ...
    try {
      ...
      result.rewritten++;
      console.log(`    ✓ Rewritten as ${newId.slice(0, 8)} (old ${entry.id.slice(0, 8)} → superseded)`);
    } catch (err) {
