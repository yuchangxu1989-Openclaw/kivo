...
import { cosineSimilarity } from '../association/association-discovery.js';
...
export async function runGraphBuild(options: GraphBuildOptions = {}): Promise<string> {
  ...
  // Load all active entries (including embeddings for semantic edges)
  const rows = db.prepare(`
    SELECT id, type, title, content, tags_json, domain, status, embedding
  ...
  // Build semantic edges using cosine similarity on embeddings
  const embeddingMap = new Map<string, Float32Array>();
  for (const row of validRows) {
    if (row.embedding && row.embedding.length > 0) {
      embeddingMap.set(row.id, new Float32Array(row.embedding.buffer, row.embedding.byteOffset, row.embedding.byteLength / 4));
    }
  }

  const SEMANTIC_THRESHOLD = 0.75;
  const embeddedIds = Array.from(embeddingMap.keys());
  let semanticEdgeCount = 0;

  if (embeddedIds.length >= 2) {
    console.log(`Building semantic edges from ${embeddedIds.length} entries with embeddings (threshold: ${SEMANTIC_THRESHOLD})...`);
    for (let i = 0; i < embeddedIds.length; i++) {
      const vecA = embeddingMap.get(embeddedIds[i])!;
      for (let j = i + 1; j < embeddedIds.length; j++) {
        const vecB = embeddingMap.get(embeddedIds[j])!;
        const similarity = cosineSimilarity(vecA, vecB);
        if (similarity >= SEMANTIC_THRESHOLD) {
          graph.addSemanticEdge(embeddedIds[i], embeddedIds[j], similarity);
          semanticEdgeCount++;
        }
      }
    }
    console.log(`  Semantic edges added: ${semanticEdgeCount}`);
  } else {
    console.log('Skipping semantic edges: fewer than 2 entries have embeddings.');
  }
