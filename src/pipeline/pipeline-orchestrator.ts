...
export class PipelineOrchestrator {
  ...
  getStatus(): { activePipelines: number; stageCounts: Record<string, number>; failureCounts: Record<string, number> } {
    const activePipelines = Array.from(this.tasks.values()).filter((task) => task.status === 'pending' || task.status === 'running').length;
    const stageCounts: Record<string, number> = {};
    const failureCounts: Record<string, number> = {};

    for (const task of this.tasks.values()) {
      for (const stage of task.completedStages) {
        stageCounts[stage] = (stageCounts[stage] ?? 0) + 1;
      }
      if (task.failedStage) {
        failureCounts[task.failedStage] = (failureCounts[task.failedStage] ?? 0) + 1;
      }
    }

    return { activePipelines, stageCounts, failureCounts };
  }
