...
export interface ValueAssessment {
  ...
  dimensions: {
    ...
    llmBlindSpot: number;
    /** Will it still be useful one month later? */
    timeliness: number;
    /** Does it transfer to different projects/scenarios? */
    crossScenario: number;
    /** Does it retain guidance after removing concrete context? */
    abstractness: number;
  };
...
export interface QualityDimensions {
  ...
  crossScenario: number;  // 跨场景性 (0-1)
  abstractness: number;   // 抽象性 (0-1)
...
export interface QualityAssessment {
  ...
  overallScore: number;
  passesGate: boolean;    // overallScore >= QUALITY_GATE_THRESHOLD
...
const QUALITY_GATE_THRESHOLD_DEFAULT = 0.6;
...
const QUALITY_GATE_THRESHOLD = parseEnvFloat(
  process.env.KIVO_QUALITY_GATE_THRESHOLD,
  QUALITY_GATE_THRESHOLD_DEFAULT,
);

const INGEST_VALUE_PROMPT = `你是一个知识价值评估引擎。判断给定知识是否填补了 LLM 的能力盲区，并执行三重测试。
...
4. timeliness（时效性）：一个月后这条信息还有效吗？临时状态、一次性调度、排查中间步骤得分低。
5. crossScenario（跨场景性）：换一个完全不同的项目/场景，它还适用吗？绑定单一项目或时间窗口得分低。
6. abstractness（抽象性）：去掉时间、人名、项目名等具体上下文后，它是否仍有指导价值？未经抽象的事件记录得分低。

所有语义判断必须由 LLM 执行。禁止使用关键词匹配、正则、FTS5 或规则引擎承担语义理解职责。

高价值类别（至少一个维度 ≥ 0.6，且三重测试必须通过）：
...
返回纯 JSON，格式：
{
  ...
  "confidence": 0.0-1.0,
  "reasoning": "一句话理由",
  "dimensions": { "privacy": 0.0-1.0, "scenarioSpecificity": 0.0-1.0, "llmBlindSpot": 0.0-1.0, "timeliness": 0.0-1.0, "crossScenario": 0.0-1.0, "abstractness": 0.0-1.0 }
}
不要包含 markdown 代码块标记。`;
...
5. timeliness（时效性）：一个月后这条信息还有效吗？临时状态、一次性调度、排查中间步骤得分低。
6. crossScenario（跨场景性）：换一个完全不同的项目/场景，它还适用吗？绑定单一项目或时间窗口得分低。
7. abstractness（抽象性）：去掉时间、人名、项目名等具体上下文后，它是否仍有指导价值？未经抽象的事件记录得分低。

价值维度（已有）：
8. privacy（私有性）：这是私有/专有知识吗？
9. scenarioSpecificity（场景特异性）：这是特定场景才需要的知识吗？
10. llmBlindSpot（LLM 已知短板）：这是 LLM 容易出错的领域吗？
...
返回纯 JSON，格式：
{
  ...
  "reasoning": "一句话理由",
  "dimensions": { "privacy": 0.0-1.0, "scenarioSpecificity": 0.0-1.0, "llmBlindSpot": 0.0-1.0, "timeliness": 0.0-1.0, "crossScenario": 0.0-1.0, "abstractness": 0.0-1.0 },
  "quality": { "atomicity": 0.0-1.0, "completeness": 0.0-1.0, "unambiguity": 0.0-1.0, "verifiability": 0.0-1.0, "timeliness": 0.0-1.0, "crossScenario": 0.0-1.0, "abstractness": 0.0-1.0 }
}
不要包含 markdown 代码块标记。`;
...
function parseValueResponse(raw: string): ValueAssessment | null {
  ...
  try {
    ...
    const dimensions = {
      ...
      timeliness: typeof dims.timeliness === 'number' ? Math.min(1, Math.max(0, dims.timeliness)) : 0,
      crossScenario: typeof dims.crossScenario === 'number' ? Math.min(1, Math.max(0, dims.crossScenario)) : 0,
      abstractness: typeof dims.abstractness === 'number' ? Math.min(1, Math.max(0, dims.abstractness)) : 0,
    };

    const isHighValue = HIGH_VALUE_CATEGORIES.includes(category)
      && dimensions.timeliness > 0
      && dimensions.crossScenario > 0
      && dimensions.abstractness > 0;
...
function applyThresholdOverride(
  ...
  if (assessment.dimensions.timeliness === 0 || assessment.dimensions.crossScenario === 0 || assessment.dimensions.abstractness === 0) return assessment;
...
export async function assessIngestValue(
  ...
  try {
    ...
    return {
      ...
      dimensions: { privacy: 0, scenarioSpecificity: 0, llmBlindSpot: 0, timeliness: 0, crossScenario: 0, abstractness: 0 },
      requiresRetry: true,
    };
  } catch {
...
function parseQualityResponse(raw: string): { quality: QualityDimensions; value: ValueAssessment } | null {
  ...
  try {
    ...
    const quality: QualityDimensions = {
      ...
      crossScenario: typeof q.crossScenario === 'number' ? Math.min(1, Math.max(0, q.crossScenario)) : 0.5,
      abstractness: typeof q.abstractness === 'number' ? Math.min(1, Math.max(0, q.abstractness)) : 0.5,
...
function computeOverallScore(dims: QualityDimensions): number {
  const scores = [dims.atomicity, dims.completeness, dims.unambiguity, dims.verifiability, dims.timeliness, dims.crossScenario, dims.abstractness];
...
function identifyFailedDimensions(dims: QualityDimensions): string[] {
  ...
  const entries: Array<[string, number]> = [
    ...
    ['crossScenario', dims.crossScenario],
    ['abstractness', dims.abstractness],
  ...
  try {
    ...
    if (result) {
      ...
      const quality: QualityAssessment = {
        ...
        overallScore,
        passesGate: overallScore >= QUALITY_GATE_THRESHOLD,
        failedDimensions,
      };
