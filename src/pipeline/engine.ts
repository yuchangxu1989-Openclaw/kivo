...
export interface PipelineEngineOptions {
  ...
  domainGoals?: DomainGoal[] | (() => DomainGoal[]);
}
...
export class PipelineEngine {
  ...
  private domainGoals?: DomainGoal[] | (() => DomainGoal[]);
  ...
  constructor(options: PipelineEngineOptions = {}) {
    ...
    this.domainGoals = options.domainGoals;
  }
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (classification.confidence < this.confidenceThreshold) {
          // Low confidence — will be filtered before persistence
        } else {
          entry.status = 'active';
        }
      ...
      context.activeEntries = entries.filter((entry) => entry.confidence >= this.confidenceThreshold);
      context.routeSummary = routeSummary;

      this.completeStage(task.id, 'classification', {
        classified: entries.length,
        lowConfidence: entries.length - context.activeEntries.length,
        routes: routeSummary,
      });
      await this.runCustomStagesAfter('classification', context);
    } catch (error) {
  ...
  private async runQualityGateStage(context: PipelineContext): Promise<void> {
    ...
    try {
      ...
      for (const entry of context.activeEntries) {
        ...
        // Entry filtered by quality gate — exclude from persistence
  ...
  private async runPersistenceStage(context: PipelineContext): Promise<void> {
    ...
    if (activeEntries.length === 0) {
      this.skipStage(task.id, 'persistence', {
        ...
        lowConfidence: entries.filter((entry) => entry.confidence < this.confidenceThreshold).length,
        filtered: entries.length - activeEntries.length,
      });
    ...
    try {
      ...
      const lowConfidenceCount = entries.filter((entry) => entry.confidence < this.confidenceThreshold).length;
      const filteredCount = entries.length - activeEntries.length;

      if (this.repository) {
        const domainGoals = this.getDomainGoals();
        const rejectedByDomainGoal: Array<{ entryId: string; reason: string }> = [];

        for (const entry of activeEntries) {
          if (entry.confidence >= this.confidenceThreshold) {
            const decision = await enforceConstraints(entry, domainGoals);
            if (!decision.allowed) {
              rejectedByDomainGoal.push({ entryId: entry.id, reason: decision.reason ?? 'domain goal constraint rejected entry' });
              continue;
            }
        ...
        if (rejectedByDomainGoal.length > 0) {
          this.emitEvent('pipeline:warning', task.id, 'persistence', {
            message: 'Filtered entries by domain goal constraints before persistence',
            rejectedEntries: rejectedByDomainGoal,
          });
        }
      } else {
        this.emitEvent('pipeline:warning', task.id, 'persistence', {
  ...
  private buildArtifactInput(task: ExtractionTask, entries: KnowledgeEntry[]): AnalysisArtifactInput {
    return {
      ...
      reviewCandidates: entries
        .filter((entry) => entry.confidence < this.confidenceThreshold)
        .map((entry) => ({
          id: `${entry.id}:review`,
  ...
  private getDomainGoals(): DomainGoal[] {
    if (!this.domainGoals) return [];
    return typeof this.domainGoals === 'function' ? this.domainGoals() : this.domainGoals;
  }
