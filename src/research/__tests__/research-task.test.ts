...
describe('FR-D03: research scheduler queue and budget', () => {
  const dbPath = join(process.cwd(), 'tmp', `research-scheduler-${Date.now()}.db`);

  function makeTask(id: string, priority: ResearchTask['priority']): ResearchTask {
    return {
      id,
      gapId: `gap-${id}`,
      gapType: 'frequency_blind_spot',
      ...
      scope: { topic: 'scheduler', boundaries: [], acquisitionMethods: ['web_search'] },
      expectedKnowledgeTypes: ['fact'],
      strategy: { steps: [], searchQueries: [] },
      completionCriteria: ['scheduled'],
      budget: { maxDurationMs: 1_000, maxApiCalls: 1 },
      priority,
      impactScore: 2,
      urgencyScore: priority === 'high' ? 3 : 1,
      blocking: priority === 'high',
      createdAt: fixedNow(),
    };
  }

  it('queues tasks by priority and caps runnable work by concurrency', () => {
    ...
    const decision = scheduler.schedule(
      [makeTask('low-1', 'low'), makeTask('high-1', 'high'), makeTask('medium-1', 'medium')],
      { maxConcurrentResearchTasks: 2, frequencyMs: 60_000, dailyBudget: 10 },
    );

    expect(decision.runnable.map((task) => task.id)).toEqual(['high-1', 'medium-1']);
    expect(decision.queued.map((task) => task.id)).toEqual(['low-1']);
    expect(decision.status).toBe('scheduled');
  });

  it('returns budget_exhausted when daily limit is reached', () => {
    rmSync(dbPath, { force: true });
    const scheduler = new ResearchScheduler({
      now: () => new Date('2026-05-01T10:00:00.000Z'),
      dbPath,
    ...
    scheduler.schedule([makeTask('first', 'high')], {
      maxConcurrentResearchTasks: 3,
      frequencyMs: 60_000,
      dailyBudget: 1,
    });

    const second = scheduler.schedule([makeTask('second', 'high')], {
      maxConcurrentResearchTasks: 3,
      frequencyMs: 60_000,
      dailyBudget: 1,
    });
    ...
    expect(second.queued.map((task) => task.id)).toEqual(['second']);
  });
});
