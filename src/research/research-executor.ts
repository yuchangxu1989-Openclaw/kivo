...
export class ResearchExecutor {
  ...
  async execute(task: ResearchTask): Promise<ResearchResult> {
    ...
    for (let index = 0; index < task.strategy.steps.length; index++) {
      ...
      try {
        const stepResult = await this.executeStepWithRetry(step, task, timeline);
  ...
  /**
   * FR-D02: Resume failed research tasks by re-running their original definitions.
   * Returns the fresh execution results in task registry order.
   */
  async resumeFailedTasks(): Promise<ResearchResult[]> {
    const failedTasks = [...this.resultHistory.values()]
      .filter((result) => result.status === 'failed')
      .map((result) => this.taskRegistry.get(result.taskId))
      .filter((task): task is ResearchTask => Boolean(task));

    const resumed: ResearchResult[] = [];
    for (const task of failedTasks) {
      resumed.push(await this.reExecute(task));
    }
    return resumed;
  }
  ...
  private async executeStepWithRetry(
    ...
    for (let attempt = 1; attempt <= MAX_STEP_ATTEMPTS; attempt++) {
      try {
        return await this.adapter.execute(step, task);
      } catch (error) {
        lastError = error;
        const msg = error instanceof Error ? error.message : String(error);
        timeline.push({
          timestamp: new Date(this.now()),
          event: attempt < MAX_STEP_ATTEMPTS ? 'step_retry_scheduled' : 'step_retry_exhausted',
          detail: `${step.id}: attempt ${attempt}/${MAX_STEP_ATTEMPTS}: ${msg}`,
        });
