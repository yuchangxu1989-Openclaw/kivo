...
import type { KnowledgeEntry, KnowledgeSource, KnowledgeType, SourcePosition, EntryStatus } from '../types/index.js';
...
export class DocumentExtractor {
  ...
  async extract(sections: ParsedSection[], source: KnowledgeSource): Promise<KnowledgeEntry[]> {
    ...
    for (let sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
      ...
      // FR-A02: Low confidence entries (< 0.7) marked as pending_review
      const status: EntryStatus = confidence < 0.7 ? 'pending_review' : 'active';

      // FR-A02: Build source position from section metadata
  ...
  private async toKnowledgeEntry(
    ...
    // FR-A02: Build source position from chunk metadata
    const position: SourcePosition = {
      paragraph: typeof chunkMetadata.index === 'number' ? chunkMetadata.index : undefined,
      section: typeof chunkMetadata.title === 'string' ? chunkMetadata.title : undefined,
    };
