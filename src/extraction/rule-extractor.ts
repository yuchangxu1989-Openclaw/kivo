...
export interface RuleEntry {
  ...
  createdAt: Date;
  updatedAt: Date;
  /** FR-A03: IDs of existing rules this entry conflicts with */
...
export interface RuleExtractorOptions {
  ...
  /** Existing rules to check for conflicts during extraction (FR-A03) */
...
export interface RuleChangeEvent {
  type: 'added' | 'modified' | 'removed' | 'conflict';
  rule: RuleEntry;
  previousRule?: RuleEntry;
  /** For conflict events: the conflicting rule */
  conflictingRule?: RuleEntry;
  /** For conflict events: reason for the conflict */
...
export class RuleExtractor {
  ...
  private existingRules: RuleEntry[];
  ...
  /** Update the set of existing rules used for conflict detection */
  setExistingRules(rules: RuleEntry[]): void {
  ...
  async extract(text: string, source: KnowledgeSource): Promise<RuleEntry[]> {
    ...
    if (this.llmProvider) {
      ...
      rules = this.extractHeuristically(text, source);
    }

    // FR-A03: Detect conflicts with existing rules and mark conflictsWith
    if (this.existingRules.length > 0) {
      for (const rule of rules) {
        const conflicts = this.findConflictsWithExisting(rule);
        if (conflicts.length > 0) {
          rule.conflictsWith = conflicts.map(c => c.id);
        }
      }
    }
  ...
  /**
   * FR-A03: Extract rules and return conflict events for any detected conflicts.
   * Emits 'conflict' type RuleChangeEvents when new rules conflict with existing ones.
   */
  async extractWithConflictEvents(
    text: string,
    source: KnowledgeSource,
  ): Promise<{ rules: RuleEntry[]; events: RuleChangeEvent[] }> {
    const rules = await this.extract(text, source);
    const events: RuleChangeEvent[] = [];

    for (const rule of rules) {
      if (rule.conflictsWith && rule.conflictsWith.length > 0) {
        for (const conflictId of rule.conflictsWith) {
          const conflictingRule = this.existingRules.find(r => r.id === conflictId);
          if (conflictingRule) {
            events.push({
              type: 'conflict',
              rule,
              conflictingRule,
              conflictReason: `Contradictory directives in scope "${rule.scene}": new rule "${rule.directive.slice(0, 60)}" conflicts with existing rule "${conflictingRule.directive.slice(0, 60)}"`,
            });
          }
        }
      }
  ...
  /**
   * FR-A03: Find existing rules that conflict with a given rule.
   * Conflict = same scene (scope) + contradictory directives.
   */
  private findConflictsWithExisting(rule: RuleEntry): RuleEntry[] {
    const conflicts: RuleEntry[] = [];
    const ruleScene = rule.scene.toLowerCase().trim();

    for (const existing of this.existingRules) {
      const existingScene = existing.scene.toLowerCase().trim();
      if (existingScene !== ruleScene) continue;
      if (areContradictory(rule.directive, existing.directive)) {
