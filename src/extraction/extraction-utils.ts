...
export function shortenKnowledgeTitle(title: string | undefined, fallbackContent = ''): string {
  ...
  if (punctuationIndex > 0) {
    ...
    if (candidate && candidate.length <= TITLE_MAX_LENGTH && (source.length > TITLE_MAX_LENGTH || trailingLength >= 8)) {
      return candidate;
    }
  }
  ...
  return shortenAtSemanticBoundary(source);
}

function shortenAtSemanticBoundary(source: string): string {
  const head = source.slice(0, TITLE_MAX_LENGTH);
  const connectorMatches = Array.from(head.matchAll(new RegExp(TITLE_CONNECTOR, 'gu')));
  for (let i = connectorMatches.length - 1; i >= 0; i--) {
    const index = connectorMatches[i].index ?? -1;
    if (index >= 2) {
      const candidate = head.slice(0, index).trim();
      if (candidate && !TITLE_ENDS_INCOMPLETE.test(candidate)) return candidate;
    }
  }

  for (let length = Math.min(TITLE_MAX_LENGTH, source.length); length >= Math.min(8, TITLE_MAX_LENGTH); length--) {
    const candidate = source.slice(0, length).trim();
    if (!candidate || TITLE_ENDS_INCOMPLETE.test(candidate)) continue;
    return candidate;
  }
