...
export class KnowledgeSearch {
  ...
  async search(query: SearchQuery | string, options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    let mode = normalizedQuery.mode ?? 'semantic';
    ...
    filtered = await this.applyDomainGoalBoost(filtered, options.domain);
  ...
  private async applyDomainGoalBoost(results: SearchResult[], domain?: SearchOptions['domain']): Promise<SearchResult[]> {
    ...
    const boosts = await boostByDomainGoal(
...
function normalizeQuery(query: SearchQuery | string): SearchQuery {
  if (typeof query === 'string') {
    return { text: query, mode: 'semantic' };
  }
