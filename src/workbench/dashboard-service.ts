...
export class DashboardService {
  ...
  async getOverview(currentWindow?: TimeWindow, previousWindow?: TimeWindow): Promise<DashboardOverview> {
    ...
    const pendingResult = await this.storage.query({ status: 'pending' }, { offset: 0, limit: 0 });
    ...
    const kpiCards: KpiCard[] = [
      ...
      this.buildKpiCard('pending-reviews', '待确认条目', pendingCount, undefined),
    ];
  ...
  private buildRecommendations(
    ...
    if (pendingReviews > 0) {
      recs.push({
        type: 'review',
        label: '审核知识',
        description: `${pendingReviews} 条知识待确认`,
        targetPath: `${this.basePath}/knowledge?status=pending`,
        priority: 3,
      });
    }
    if (totalEntries === 0) {
