...
describe('FR-W02 AC1: filter by type, status, source, domain + pagination', () => {
  const entries = [
    ...
    makeEntry({ id: 'a3', type: 'fact', status: 'active', source: makeSource('research'), domain: 'ml' }),
    makeEntry({ id: 'a4', type: 'decision', status: 'active', source: makeSource('manual'), domain: 'ml' }),
    makeEntry({ id: 'a5', type: 'fact', status: 'active', source: makeSource('document'), domain: 'infra' }),
  ];
