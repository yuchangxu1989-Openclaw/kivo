...
术语条目的输出要求：
- type 必须为 "fact"
- domain 必须为 "system-dictionary"
- title 格式为术语名本身（如 "KIVO"、"焰崽"）
- content 为完整定义（包含全称、缩写、含义）
- confidence >= 0.7
...
输出格式：JSON 数组，每个元素包含：
{
  ...
  "confidence": 0.0-1.0,
  "tags": ["标签1", "标签2"],
  "domain": "知识所属领域（术语条目必须为 system-dictionary）"
}
...
// --- Dedup check for dictionary entries (FR-3 AC4) ---

function isDuplicateDictEntry(db, title) {
  const existing = db.prepare(
    "SELECT title FROM entries WHERE domain = 'system-dictionary' AND status = 'active'"
  ).all();
  const newTitle = (title || '').toLowerCase().trim();
  for (const row of existing) {
    const existingTitle = (row.title || '').toLowerCase().trim();
...
function writeEntriesToDb(dbPath, entries) {
  ...
  const insert = db.prepare(`
    INSERT INTO entries (id, type, title, content, summary, source_json, confidence, status, tags_json, domain, embedding, created_at, updated_at, version)
  ...
  const tx = db.transaction((items) => {
    for (const entry of items) {
      try {
        // FR-3 AC4: Dedup check for dictionary entries
        if (entry.domain === 'system-dictionary') {
          if (isDuplicateDictEntry(db, entry.title)) {
            console.log(`[extract-worker] Skipping duplicate dict entry: ${entry.title}`);
            continue;
          }
        }
    ...
    for (let i = 0; i < sessionMsgs.length; i += 10) {
      ...
      try {
        ...
        if (validEntries.length > 0) {
          // FR-3 AC5: Auto-generate embeddings for dictionary entries
          for (const entry of validEntries) {
            if (entry.domain === 'system-dictionary') {
              const embText = `${entry.title}\n${entry.content}`;
              entry._embedding = await generateEmbedding(embText);
            }
          }
