 ...
 * 2. 用本机 Ark 兼容层 doubao-embedding-vision-251215 补全 system-dictionary 条目的 embedding
 */
...
const ARK_URL = 'http://localhost:9876/v1/embeddings';
const MODEL = 'doubao-embedding-vision-251215';
const EXPECTED_BLOB_SIZE = 2048 * 4;
...
async function backfillDictEmbeddings() {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (!embedding || !Array.isArray(embedding)) { console.error(`  [${row.title}] No embedding`); continue; }
      const buffer = Buffer.from(new Float32Array(embedding).buffer);
      db.prepare('UPDATE entries SET embedding = ? WHERE id = ?').run(buffer, row.id);
      success++;
      console.log(`  [${row.title}] OK (dim=${embedding.length})`);
...
async function verify() {
  ...
  const count = db.prepare("SELECT COUNT(*) as n FROM entries WHERE domain = 'system-dictionary' AND length(embedding) = ?").get(EXPECTED_BLOB_SIZE);
  db.close();
