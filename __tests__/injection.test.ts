...
describe('RelevanceScorer', () => {
  ...
  it('assigns 0 score when query has no matching keywords', async () => {
    ...
    const scored = await scorer.score('', entries);
  ...
  it('respects token budget and truncates', async () => {
    const entries = Array.from({ length: 3 }, (_, i) =>
      makeEntry({
        id: `big-${i}`,
        title: `Entry ${i}`,
        content: 'Entry content about topic ' + 'x'.repeat(60),
    ...
    expect(response.tokensUsed).toBeLessThanOrEqual(40);
    expect(response.entries.length).toBeLessThan(3);
