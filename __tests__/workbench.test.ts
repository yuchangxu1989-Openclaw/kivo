...
describe('DashboardService (FR-W01)', () => {
  it('AC1+AC2: returns KPI cards with trend data', async () => {
    ...
    const storage = makeStorage([makeEntry({ status: 'pending', id: 'p1' }), makeEntry({ id: 'a1' })]);
...
describe('EntryOperationService (FR-W06)', () => {
  it('AC1: changes pending to active via confirm', async () => {
    const entry = makeEntry({ id: 'p1', status: 'pending' });
    const storage = makeStorage([entry]);
  ...
  it('AC2: rejects invalid operation', async () => {
    const entry = makeEntry({ id: 'a1', status: 'archived' });
    const storage = makeStorage([entry]);
    const svc = new EntryOperationService({ storage });
    await expect(
...
describe('Status Transitions', () => {
  it('resolveNewStatus maps correctly', () => {
    ...
    expect(resolveNewStatus('pending', 'reject')).toBe('archived');
    expect(resolveNewStatus('active', 'deprecate')).toBe('deprecated');
  });

  it('STATUS_TRANSITIONS covers all statuses', () => {
    const statuses: EntryStatus[] = ['active', 'pending', 'rejected', 'draft', 'deprecated', 'archived', 'superseded'];
