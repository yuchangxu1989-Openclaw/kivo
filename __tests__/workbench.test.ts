...
describe('ActivityStreamService (FR-W04)', () => {
  ...
  it('AC4: cursor-based catch-up after reconnection', () => {
    ...
    const result = svc.query({ afterCursor: second.eventId, limit: 10 });
    expect(result.events.length).toBe(1);
    expect(result.events[0].summary).toBe('Third');
  });

  it('AC4: reports history loss when Last-Event-ID is outside ring buffer', () => {
    const svc = new ActivityStreamService(undefined, { cacheLimit: 2 });
    const first = svc.push({ type: 'entry:created', timestamp: new Date(), summary: 'First' });
    svc.push({ type: 'entry:updated', timestamp: new Date(), summary: 'Second' });
    svc.push({ type: 'entry:created', timestamp: new Date(), summary: 'Third' });

    const result = svc.query({ afterCursor: first.eventId, limit: 10 });
    expect(result.historyLost).toBe(true);
