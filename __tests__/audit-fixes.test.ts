...
describe('Fix 2: EntryStatus only active', () => {
  it('only accepts active as valid EntryStatus value', () => {
...
describe('Fix 3: SQLiteProvider.search() filters', () => {
  ...
  beforeEach(async () => {
    ...
    await provider.save({ ...base, id: 'e1', type: 'fact', title: 'fact about users', content: 'users count is 1500', confidence: 0.9, status: 'active', domain: 'analytics' });
    ...
    await provider.save({ ...base, id: 'e3', type: 'fact', title: 'fact about revenue', content: 'revenue is growing for users', confidence: 0.7, status: 'active', domain: 'analytics' });
  });
  ...
  it('filters by status', async () => {
    const results = await provider.search({ text: 'users', filters: { status: ['active'] } });
...
describe('Fix 4: Extractor minConfidence threshold', () => {
  it('all entries are active regardless of confidence', async () => {
    ...
    // All entries are always active now
