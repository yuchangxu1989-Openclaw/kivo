...
describe('FR-B01: Structured Knowledge Storage', () => {
  ...
  it('AC2: supports all entry statuses', async () => {
    const statuses = ['active', 'superseded', 'deprecated', 'pending', 'archived'] as const;
  ...
  it('query filters by type, domain, status, tags, confidence', async () => {
    ...
    await store.save(makeEntry({ id: 'e3', type: 'fact', domain: 'ops', status: 'pending', tags: ['a'], confidence: 0.3 }));
    ...
    const byStatus = await store.query({ status: 'pending' });
